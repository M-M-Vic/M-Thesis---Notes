...
Setting $x=y=z$ in the fundamental equation~\eqref{eq:B2:fundamental}, the
...
\begin{align}
    ...
    \label{eq:B2:Pzz}
\end{align}
the same formula as~\eqref{eq:A:Pzz}.  Since
$\tfrac{d}{dz}P(z,z)\big|_{z=1}=\mathbb{E}[N_1]+\mathbb{E}[N_2]$,
...
At $y=1$ the exponent $\beta(1)=(1-1)(\lambda{\cdot}1-\mu)/(\gamma_1{\cdot}1)=0$,
so the jockeying coefficient $\gamma_1 xy(x-y)|_{y=1}=\gamma_1 x(x-1)$, and
equation~\eqref{eq:B2:fundamental} becomes a first-order linear ODE in $x$:
\begin{align}
    \begin{split}
...
and integrating from $0$ to $x$ (the boundary term vanishes since $0^{\mu/\gamma_1}=0$):
\begin{align}
    \begin{split}
...
$P_y(1)=\rho\gamma_1/({\mu\,e^{\lambda_1/\gamma_1}\,\mathcal{J}(1)})$.
...
factor $(\lambda_1-\mu)<0$ (since $\rho_1<1$), confirming that
...
\textbf{Little's Law} applied to the waiting sub-system of each class:
\begin{align}
    \begin{split}
        \mathbb{E}[W_1]
    ...
    \label{eq:B2:EW}
\end{align}
As $\gamma_1\to0^+$ both recover the Model~$A$ values~\eqref{eq:A:EW1}--\eqref{eq:A:EW2}.
