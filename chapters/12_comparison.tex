...
three full-rate models; the exhaustive sweep-by-sweep evidence is deferred to
...
Throughout, waiting times follow from \emph{Little's Law},
$\mathbb{E}[W_i]=\mathbb{E}[N_i]/\lambda_i$, applied per class. For Model-$C_2$
we use the \emph{offered} rate $\lambda_i$, not the carried throughput, so
...
load than $C_2$ ($\pi_0=\piZeroCHSeventy$, $\mathbb{E}[N]=\ENCHSeventy$ at $\rho=0.70$). The full
sweep suite is collected in Appendix~\ref{app:numerics:sweep}.
...
does not certify faster service; its attrition component is the loss fraction $L_1$ of
\S\ref{subsec:comp:abandonment}. Second, jockeying actively \emph{harms} class~2: at $\rho=0.70$,
