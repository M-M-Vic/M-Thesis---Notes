...
\begin{figure}[H]
  ...
  the baseline $A$, the jockeying pair $B_2$, $B_2^1$ ($\gamma_1=0.5$), and the
  abandonment pair $C_2$, $C_2^1$ ($\theta_1=0.5$) --- at $\mu=1$ and
  $\lambda_1{:}\lambda_2=3{:}4$; the vertical line marks the canonical $\rho=0.70$.
  \emph{Top-left:} total queue $\mathbb{E}[N]$ --- $A$, $B_2$ and $B_2^1$ coincide because
  jockeying conserves $N$ (Corollaries~\ref{cor:A:pi0}, \ref{cor:B21:pi0}), while $C_2$ and
  $C_2^1$ lie strictly below because abandonment removes customers ($C_2^1$ by less).
  ...
  (Models~$A$, $B_2$, $B_2^1$) shown dotted; $C_2$ and $C_2^1$ sit above it for every $\rho$,
...
\begin{figure}[H]
  ...
  $B_2^1$ lie exactly on the parabola $\rho(1-\rho)$ (Corollaries~\ref{cor:A:pi0},
  \ref{cor:B2:pi00}, \ref{cor:B21:pi0}: jockeying preserves $N$, hence the busy-but-empty
  probability), whereas the abandonment models $C_2$ and $C_2^1$ rise strictly above it ---
  ...
  than the head-of-line rate ($C_2^1$).}
  \label{fig:comp:pi00}
\end{figure}
...
The two head-of-line variants $B_2^1$ and $C_2^1$ added to
Figures~\ref{fig:comp:sweep_en}--\ref{fig:comp:pi00} confirm that the qualitative split
survives the head-of-line simplification but in a muted form. Model-$B_2^1$ conserves $N$
...
Model-$C_2^1$ tracks $C_2$ but bleeds the priority queue more gently --- at $\rho=0.70$ it
...
(eq.~\eqref{eq:comp:lossfrac}): in $C_2$ at $\rho=0.70$ a fraction $L_1=0.232$ of class-1
arrivals leaves by abandonment rather than by service, so the reported
$\mathbb{E}[W_1]=0.4639$ averages the queue residence over a population nearly a quarter of
which never reaches the server. A strict like-for-like comparison would instead report the
conditional mean time-to-service of \emph{served} customers, which the count-based
$\mathbb{E}[W_1]=\mathbb{E}[N_1]/\lambda_1$ does not isolate; for the lossless Models $A$ and
$B_2$ ($L_1=0$) the distinction is vacuous, and the caveat bites only for $C_2$ and $C_2^1$.
...
\begin{figure}[H]
  ...
  benefit $\mathbb{E}[W_2]-\mathbb{E}[W_1]$, for Models $A$, $B_2$ and $B_2^1$
  ($\gamma_1=0.5$), and $C_2$ and $C_2^1$ ($\theta_1=0.5$).}
...
\end{equation}
the flat line at $0.300=1-\rho$ in the panel. The two mechanisms break this invariance in
opposite directions, and \emph{monotonically} in $\alpha$: jockeying makes the ratio strictly
decreasing (from $0.188$ at $\alpha=0.1$ to $0.031$ at $\alpha=0.9$), since a larger priority
share feeds the $1\!\to\!2$ drain harder; abandonment makes it strictly increasing ($0.214$
to $0.357$), since a larger priority share is exactly where the abandonment valve sheds the
most class-1 work. The head-of-line variants $B_2^1$ and $C_2^1$ track their parents and are
likewise monotone. No curve turns over: each ratio is extremised at a \emph{boundary} of the
swept range $[0.1,0.9]$, not at an interior $\alpha^\ast$, so the dotted markers sit at the
endpoints ($\alpha=0.1$ for $A$ and the jockeying models, $\alpha=0.9$ for the abandonment
models) and record this monotonicity rather than an interior optimum.
...
the three reference loads for the full-rate $C_2$ and the head-of-line $C_2^1$; the
...
$C_2^1$ sheds uniformly less than the full-rate $C_2$.}
\label{tab:comp:loss}
\begin{tabular}{lrrrr}
\hline
 & \multicolumn{2}{c}{\textbf{Model-$C_2$}} & \multicolumn{2}{c}{\textbf{Model-$C_2^1$}} \\
$\rho$ & $L_1$ & $L$ & $L_1$ & $L$ \\
\hline
...
\subsection{Head-of-line versus full-rate mechanisms (Models \texorpdfstring{$B_2^1$}{B21} and \texorpdfstring{$C_2^1$}{C21})}
...
\begin{figure}[H]
  ...
  $\mathbb{E}[N_1]$ and $\pi_0$ for $B_2$ (full) versus $B_2^1$ (head-of-line). The
  ...
  $\mathbb{E}[N_1]$ and $\pi_0$ for $C_2$ (full) versus $C_2^1$ (head-of-line). The
  head-of-line valve is weaker, so $C_2^1$ retains a slightly longer priority queue and a
...
(Corollary~\ref{cor:B21:pi0}), the milder class-1 drain of $B_2^1$ is mirrored by a milder
class-2 inflation: at $\rho=0.70$, $\mathbb{E}[N_2]=1.4583$ for $B_2^1$ against $1.4788$ for
$B_2$, and the priority ratio $\mathbb{E}[W_2]/\mathbb{E}[W_1]$ is $6.25$ for $B_2^1$ versus
...
to $0.3696$ and compresses $\mathbb{E}[N]$ to $0.8358$, while the head-of-line $C_2^1$ stops
...
($\mathbb{E}[W_2]/\mathbb{E}[W_1]=3.50$ for $C_2^1$ against $3.33$ for $A$ and $3.76$ for
