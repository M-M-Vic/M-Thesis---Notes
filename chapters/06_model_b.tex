...
\end{equation}
Following the framework of \S\ref{sssec:moc}, the method of characteristics converts this PDE into the characteristic system of ODEs
\begin{equation}
...
Under stability ($\rho<1$), the exponent $E(x,y)$ is negative throughout the interior of the unit square.  To see this, note from~\eqref{eq:B:E} that $E(x,y)$ is a function of $C_1=\gamma_2 x+\gamma_1 y$; at $y=0,x=0$ one has $E(0,0)$ undefined, but for $x,y\in(0,1)$ with $\rho<1$ an explicit sign computation shows $E<0$ (an analogous calculation appears in the exponent of Model~$B_2$'s integrating factor, defined in \S\ref{sec:model_b2}).  Consequently $(x-y)^{E(x,y)}\to\infty$ as $x\to y^-$, producing the branch singularity that prevents a direct evaluation at $x=y$.

Setting $\gamma_2=0$ in~\eqref{eq:B:Ph} gives $C_1=\gamma_1 y$ (a constant along each vertical characteristic), and $P_h|_{\gamma_2=0}$ reduces to
\begin{equation}
    P_h(x,y)\big|_{\gamma_2=0}
    = F(\gamma_1 y)\cdot \exp\!\left(\frac{\lambda_1}{\gamma_1}x\right)\cdot x^{-\mu/(\gamma_1 y)}\cdot(x-y)^{E(x,y)|_{\gamma_2=0}},
    \label{eq:B:Ph_B2limit}
\end{equation}
which coincides with the reciprocal of the integrating factor used in Model~$B_2$ (see \S\ref{sec:model_b2}).  The two analyses are therefore structurally identical at $\gamma_2=0$: the homogeneous solution of Model~$B$ degenerates to the inverse integrating factor of Model~$B_2$.

\subsubsection*{Particular solution via variation of parameters}

Variation of parameters (see \S\ref{sssec:vop}) exploits the homogeneous solution as a fundamental solution for the inhomogeneous equation.  Restricting attention to a fixed characteristic curve $y(x)=(C_1-\gamma_2 x)/\gamma_1$, the full fundamental equation~\eqref{eq:gen:fundamental_equation} becomes a first-order ODE in $x$:
\begin{equation}
...
\begin{align}
    ...
    \label{eq:B:Gdef}
\end{align}
Since $P_h(x,y(x))$ satisfies the homogeneous equation $\frac{dP_h}{dx}=f(x;C_1)P_h$, it is the fundamental solution of~\eqref{eq:B:ode_char}.  The general solution of the full inhomogeneous equation is therefore
\begin{equation}
...
When restricted to the characteristic, $G(t,y(t))$ in~\eqref{eq:B:Gdef} simplifies.  Substituting $G$ directly into~\eqref{eq:B:hdef}:
\begin{align}
    h(t;C_1)
...
\end{align}
Along the diagonal characteristic with $C_1=(\gamma_1+\gamma_2)z$, the parametrisation satisfies $y(t;z)>t$ for all $t\in[0,z)$ (since $y(0;z)=(\gamma_1+\gamma_2)z/\gamma_1 > z$ and $y(z;z)=z$), so $t-y(t)<0$.  Writing $t-y(t) = -(y(t)-t)$ converts the negative denominator and gives
\begin{equation}
...
\end{equation}
Writing $\lambda=\lambda_1+\lambda_2$, the bracket factors as $(\mu-\lambda x)(x-1)$; cancelling $(x-1)$ from both sides yields
\begin{equation}
    P(x,x) = \frac{\mu\,\pi(0,0)}{\mu-\lambda x} = \frac{\pi(0,0)}{1-\rho x}.
    \label{eq:B:Pxx}
\end{equation}
Setting $x=1$ and using $P(1,1)=1-\pi_0$ together with the idle-state balance $\lambda\pi_0=\mu\pi(0,0)$ gives:
\begin{equation}
    \pi_0 = 1-\rho, \qquad \pi(0,0) = \rho(1-\rho).
    \label{eq:B:pi00}
\end{equation}
This recovers the same values as Model~$A$~\eqref{eq:A:pi0_pi00}: jockeying conserves the total-customer process and therefore cannot change the marginal idle probability.  The same identities will reappear in Models~$B_2$ and~$C_2$ for the same reason~\eqref{eq:gen:pi0_noaband}.
