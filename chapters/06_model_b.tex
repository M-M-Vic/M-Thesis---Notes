...
\textbf{Stability.} $\rho = (\lambda_1+\lambda_2)/\mu < 1$. Jockeying conserves the total customer count and does not affect this condition.
...
\begin{equation}
    ...
    \label{eq:B:KMN}
\end{equation}
(For $N$: substituting $K$ and $M$ into the $x^1$ match gives
$N = -\gamma_1(\lambda_1+\lambda_2+\mu)+\lambda_2 C_1+KC_1-M(\gamma_1+\gamma_2)$;
expanding $KC_1$ and $-M(\gamma_1+\gamma_2)$ and collecting yields~\eqref{eq:B:KMN}.)
...
\begin{equation}
    ...
    \label{eq:B:simplify}
\end{equation}
so $\ln[(\gamma_1+\gamma_2)x-C_1] = \ln\gamma_1 + \ln(x-y)$.  The constant
$\ln\gamma_1$ is absorbed into a redefined arbitrary function $F(C_1)$.  Reading
off the exponent of $(x-y)$:
\begin{equation}
...
Write $\lambda = \lambda_1+\lambda_2$ and $\Gamma = \gamma_1+\gamma_2$. Along any
...
\begin{equation}
    ...
    \label{eq:B:E_u}
\end{equation}
(For $(x,y)\in(0,1)^2$ we have $u = \gamma_2 x+\gamma_1 y \in (0,\Gamma)$, and
$E$ is bounded and continuous on this interval.)  Multiplying through by
...
$\lim_{x\to z^-}$ of~\eqref{eq:B:integral_rep_body}; an asymptotic analysis near
$t=z$ (expanding $y(t;z)-t\sim(\Gamma/\gamma_1)(z-t)$) confirms that the
$0\cdot\infty$ limit is finite and equals $\pi(0,0)/(1-\rho z)$.
...
\]
Substituting into the fundamental equation and dividing by $\gamma_1 x(x-y(x))$
...
% ─────────────────────────────────────────────────────────────────────────────
\medskip\noindent\textbf{Step~5: Empty-state probabilities and reduction to the Volterra equation.}

Before evaluating on the diagonal, we determine $\pi_0$ and $\pi(0,0)$.  Setting
$x=y$ in the fundamental equation~\eqref{eq:gen:fundamental_equation} with
$\theta_1=\theta_2=0$: the jockeying terms both carry the factor $(x-y)$,
which vanishes; the boundary term $\mu(x-y)P_y(y)$ also vanishes.  What remains is
\begin{equation}
    \bigl[(\lambda_1+\lambda_2+\mu)x - \mu - (\lambda_1+\lambda_2)x^2\bigr]P(x,x)
    = -\mu(1-x)\,\pi(0,0).
    \label{eq:B:diagonal_ode}
\end{equation}
Writing $\lambda=\lambda_1+\lambda_2$, the bracket factors as
$(\mu-\lambda x)(x-1)$; cancelling $(x-1)$ from both sides (valid for $x\neq1$):
\begin{equation}
    P(x,x) = \frac{\mu\,\pi(0,0)}{\mu-\lambda x} = \frac{\pi(0,0)}{1-\rho x}.
    \label{eq:B:Pxx}
\end{equation}
Setting $x=1$ and using $P(1,1)=1-\pi_0$ together with the idle-state
balance $\lambda\pi_0=\mu\pi(0,0)$:
\begin{equation}
    \pi_0 = 1-\rho, \qquad \pi(0,0) = \rho(1-\rho).
    \label{eq:B:pi00}
\end{equation}
This recovers the same values as Model~$A$~\eqref{eq:A:pi0_pi00}: jockeying
conserves the total-customer process and cannot change the marginal idle
probability.  The same identities reappear in Models~$B_2$ and~$C_2$
for the same reason~\eqref{eq:gen:pi0_noaband}.

Next, we simplify the forcing function $h$ on the characteristic.  Substituting
...
As discussed in Step~2, taking $\lim_{x\to z^-}$ of~\eqref{eq:B:integral_rep_body}
(which equals $P(z,z)=\pi(0,0)/(1-\rho z)$ by~\eqref{eq:B:Pxx}), and
substituting~\eqref{eq:B:h_simplified}, the known and unknown parts separate:
\begin{align}
    \frac{\pi(0,0)}{1-\rho z}
    ...
    \label{eq:B:split}
\end{align}
where $P_h(z,z)$ denotes the limiting ratio of the homogeneous solution
(absorbed into the prefactor from the $\lim_{x\to z^-}$ computation).
Defining the kernel and the known term,
\begin{align}
...
equation~\eqref{eq:B:split} becomes
\begin{equation}
    \mathcal{F}(z) = \int_0^z \mathcal{K}(z,t)\,P_y\!\bigl(y(t;z)\bigr)\,dt,
    \label{eq:B:rhs}
\end{equation}
confirming Claim~\ref{clm:B:volterra}.  This is a Volterra integral equation of
the first kind for the unknown $P_y$. The kernel $\mathcal{K}$ depends on $P_h$
through both arguments and does not simplify for general $(\gamma_1,\gamma_2)$,
so~\eqref{eq:B:rhs} does not yield a closed form.  Resolving it would require
Volterra inversion methods well beyond the scope of this thesis.
\end{proof}
