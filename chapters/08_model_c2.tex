\section{Analysis of Model~C$_2$ ($\theta_1>0,\,\gamma_1=\gamma_2=\theta_2=0$)}\label{sec:model_c2}
...
Setting $x=y=z$ in the theorem formula~\eqref{eq:C2:Pxy}, the integrating
factor $\mu_I(z;z) \propto z^\alpha(1-z)^{\beta(z)}$ is well-defined but the
abandonment convection $\theta_1 xy(x-1)$ does \emph{not} vanish at $x=y=z$
(since $(x-1)|_{x=z}\neq0$).  Consequently, the diagonal equation becomes:
\begin{align}
    \begin{split}
    ...
    \label{eq:C2:diag}
\end{align}
and the unknown $\partial_x P(z,z)$ prevents a closed form for $P(z,z)$.
Hence $\mathbb{E}[N]=\mathbb{E}[N_1]+\mathbb{E}[N_2]$ is strictly less than
$\rho^2/(1-\rho)$: abandonment removes customers and reduces both means
relative to Model~A.

\subsubsection*{Means from the theorem formula}

Differentiating the theorem formula~\eqref{eq:C2:Pxy} with respect to $x$
and $y$ respectively yields the means; the limit $(x,y)\to(1,1)$ is
approached along a non-singular path:
\begin{align}
    \begin{split}
        \mathbb{E}[N_1]
        &\;=\; \lim_{\varepsilon\to0^+}
        ...
        \mathbb{E}[N_2]
        &\;=\; \lim_{\varepsilon\to0^+}
        \frac{P(1{-}\varepsilon,\,1{-}\varepsilon)-P(1{-}\varepsilon,\,1{-}2\varepsilon)}
             {\varepsilon}.
    \end{split}
    \label{eq:C2:EN_def}
\end{align}
Both integrals $\mathcal{I}_1,\mathcal{I}_2$ in~\eqref{eq:C2:Pxy} depend on
$\theta_1$ through the exponent $\beta(y)=\lambda_2(1-y)/\theta_1$; no closed
form exists for the individual means.
