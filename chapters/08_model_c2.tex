...
\begin{figure}[H]
    ...
    \resizebox{0.65\textwidth}{!}{%
        \input{figures/model-c2-diagram.tikz}%
    }
    \caption{State-transition diagram for Model~C$_2$ on state space $S$.
    Dotted arrows indicate Class-1 abandonment at rate $\theta_1 n_1$ per state;
    there is no jockeying and no Class-2 abandonment.
    Note the absence of a dotted arrow at $(0,n_2)$: when $n_1=0$ no Class-1
    customer is waiting, so the abandonment rate is zero at that boundary.}
    \label{fig:model-c2-diagram}
\end{figure}
