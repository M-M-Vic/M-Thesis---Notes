...
Abandonment reduces the load: $\mathbb{E}[N_1]+\mathbb{E}[N_2] < \rho^2/(1-\rho)$
for $\theta_1>0$.
The individual means are obtained from $P(x,y)$ by differentiation at $(1,1)$
(approached via a non-singular path), or directly from the CTMC.

\textbf{Little's Law with abandonment.}
The relation $\mathbb{E}[N_i] = \lambda_i\,\mathbb{E}[W_i]$ requires no
Poisson assumption: it is a universal identity between the stationary mean
number of customers present, the arrival rate, and the mean time spent in
queue.  For Model~C$_2$, $\lambda_i$ must be the \emph{arrival} rate of all
class-$i$ customers (including those who will abandon), so $\mathbb{E}[W_i]$
represents the mean waiting time averaged over served and abandoned customers
alike.  The throughput $\mu(1-\pi_0) < \lambda_1+\lambda_2$ should \emph{not}
replace $\lambda_i$ in Little's Law.

As $\theta_1\to0^+$, both $\mathbb{E}[N_1]$ and $\mathbb{E}[N_2]$ recover the
Model~A expressions~\eqref{eq:A:EN1}--\eqref{eq:A:EN2}.
