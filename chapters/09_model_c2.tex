...
\textbf{Stability.} Unlike the models without abandonment, stability here is non-trivial, and rests on two structural observations that also drive the analysis below. First, the class-$1$ queue, taken on its own, is an $M/M/1{+}M$ (Erlang-A) subsystem with rates $\lambda_1,\mu,\theta_1$, which is positive recurrent for \emph{every} load (\S\ref{ssec:erlangA}); its busy period $B_C$ is therefore always well defined. Second, class-$2$ sees an $M/G/1$ queue whose service time is one such class-$1$ busy period $B_C$ (made precise in \S\ref{ssec:C2:prob}). The only constraint on stability comes from this second queue, which is stable exactly when its utilisation stays below one:
\begin{equation}
...
Equation~\eqref{eq:C2:fundamental} carries two unknowns: $P(x,y)$ on $[0,1]^2$ and its boundary trace $P_y(y)$. The proof below treats it as an ODE in $x$: an integrating factor reduces it to quadrature, boundedness of $P$ as $x\to1$ pins the unknown $P_y(y)$, and a final integration recovers $P(x,y)$.

The following theorem states the closed form. Its proof, given immediately afterwards in \S\ref{ssec:C2:proof}, is purely analytical; \S\ref{ssec:C2:prob} then re-derives the boundary function $P_y(y)$ probabilistically as an independent cross-check.
...
\end{align*}
The numerator of $p(x;y)$ is the quadratic $A(x,y)\equiv(\lambda_1+\lambda_2+\mu)x-\mu-\lambda_1 x^2-\lambda_2 xy$, the kernel polynomial common to every model. Its partial-fraction decomposition is
\begin{equation*}
...
We pin $P_y(y)$ by the same boundedness argument used for Model-$A$, watching $P(x,y)$ as $x\to 1$ for fixed $y\in[0,1)$: there $\beta(y)>0$, so $\mu_I(x;y)\to0$. (At $y=1$ the exponent $\beta(1)=0$ and $\mu_I(\,\cdot\,;1)$ no longer vanishes at $x=1$; the value $P_y(1)$ is then recovered by continuity, $y\to1^-$.) Since the left-hand side $P(x,y)$ of~\eqref{eq:C2:Pxy_implicit} stays bounded while $1/\mu_I(x;y)\to\infty$, the integral must vanish in the limit:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:C2:Py}
\end{equation}
This determination uses no probabilistic structure: it reaches~\eqref{eq:C2:Py} through the Beta-integral identity~\eqref{eq:C2:Bc_Kummer} alone---the analytic shadow of the $M/G/1$/busy-period decomposition that \S\ref{ssec:C2:prob} makes explicit.
...
\end{align*}
with $\alpha,\beta(y)$ as in~\eqref{eq:C2:exponents}. These are the partial (upper limit $x$) counterparts of the full integrals $\mathcal{J}_0,\mathcal{J}_1$ of Step~(ii): $\mathcal{H}_2(1,y)=\mathcal{J}_1(y)$ exactly, whereas $\mathcal{H}_1(1,y)\neq\mathcal{J}_0(y)$ (its $(1-t)$ exponent is $\beta(y)$, not $\beta(y)-1$). The mismatch is harmless, as $\mathcal{H}_1$ and $\mathcal{H}_2$ only ever appear in the combination $\zeta\,\mathcal{H}_1-(1-\zeta)\,\mathcal{H}_2$.
...
\begin{proof}[Proof of Corollary~\ref{cor:C2:pi00}]
    ...
    To fix $\pi_0$, we use the $M/G/1$ structure identified in the stability analysis above and developed in \S\ref{ssec:C2:prob}. By the $M/G/1$ utilisation identity, the long-run fraction of time this queue is busy is $\rho_B=\lambda_2\,\mathbb{E}[B_C]$, and ``idle'' means no class-$2$ customer is present, so
    \begin{equation*}
...
\end{align*}
so $\mathbb{P}(\text{busy},N_1=0)=\pi(0,0)/(1-\lambda_2\mathbb{E}[B_C])$. Substituting back, the factor $(1-\lambda_2\mathbb{E}[B_C])$ cancels and we recover exactly~\eqref{eq:C2:Py}, in agreement with the analytical route. The result also matches the probabilistic argument for Model-$A$ (Equation~\eqref{eq:A:Py}); the only difference is the effective service time, now the busy period of an $M/M/1{+}M$ (Erlang-A) subsystem rather than of an ordinary $M/M/1$.

Finally, this route exhibits the Kummer fraction on the probabilistic side. The busy-period LST that the Pollaczek--Khinchine formula feeds on is, from the Erlang-A analysis (\S\ref{ssec:erlangA}, Equation~\eqref{eq:erlangA:cf_kummer}), the ratio of confluent hypergeometric functions
\begin{equation*}
