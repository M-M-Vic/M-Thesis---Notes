...
\begin{theorem}\label{thm:C2:PGF}
    ...
    \end{align*}
    $\widetilde{B}_C(s)$ is the LST of the busy period of the class-$1$ $M/M/1{+}M$ subsystem given by Equation~\eqref{eq:erlangA:bp_lst}, and $\pi(0,0)$ is given by Corollary~\ref{cor:C2:pi00} below.
\end{theorem}
...
The surviving abandonment factor $(x-1)$ places the operative singularity on the boundary $x=1$. Dividing Equation~\eqref{eq:C2:fundamental} by the coefficient $\theta_1 xy(x-1)$ of the derivative puts it in the standard form (Equation~\eqref{eq:ode:standard}):
\begin{align*}
...
We determine $P_y(y)$ by imposing boundedness of $P(x,y)$ as $x\to 1$, for fixed $y\in[0,1)$: we have $\beta(y)>0$, so $\mu_I(x;y)\to0$. Since the left-hand side $P(x,y)$ of Equation~\eqref{eq:C2:Pxy_implicit} stays bounded while $1/\mu_I(x;y)\to\infty$, the integral must vanish in the limit:
\begin{equation}
...
\end{equation*}
where $t^{\alpha}/t=t^{\alpha-1}$ and $(1-t)^{\beta(y)}/(t-1)=-(1-t)^{\beta(y)-1}$ flips the sign and lowers the exponent. The constant prefactor $-\mu/(\theta_1 y)$, nonzero for $y\in(0,1]$, divides out of Equation~\eqref{eq:C2:boundedness}, leaving
\begin{equation*}
...
\begin{equation}
    ...
    \label{eq:C2:Py}
\end{equation}
This reaches $P_y(y)$ analytically through the Beta-integral (Equation~\eqref{eq:C2:Bc_Kummer}) alone, without requiring the probabilistic argument in \S\ref{ssec:C2:prob}.
...
\end{align*}
which has the same form as the RHS of Model-$A$, now with the more involved service time $B_C$. With $p(x;y)$ and $\mu_I(x;y)$ as in Equations~\eqref{eq:C2:pfd} and~\eqref{eq:C2:integrating_factor}, the standard-form source becomes
\begin{equation*}
...
\end{align*}
with $\alpha,\beta(y)$ as in Equation~\eqref{eq:C2:exponents}. They are related to the full integrals of Step-(ii) in the following way:  $\mathcal{H}_2(1,y)=\mathcal{J}_1(y)$ exactly, and $\mathcal{H}_1(1,y)\neq\mathcal{J}_0(y)$ (its $(1-t)$ exponent is $\beta(y)$, not $\beta(y)-1$).
...
\begin{proof}[Proof of Corollary~\ref{cor:C2:pi00}]
    ...
    We again use the $M/G/1$ structure identified in \S\ref{ssec:C2:prob}. The long-run fraction of time this queue is busy is $\rho_B=\lambda_2\,\mathbb{E}[B_C]$, and ``idle'' refers exclusively to no class-$2$ customers being present. So
    \begin{equation*}
...
The boundary function $P_y(y)$ admits a second derivation, using the same probabilistic argument seen in \S\ref{sssec:A:probabilistic} that does not require imposing boundedness. By the \emph{law of total expectation}, splitting $P_y(y)=\mathbb{E}[\mathbf{1}\{\text{busy},N_1=0\}\,y^{N_2}]$ over the event $\{\text{busy},N_1=0\}$ factors it into the mass of that event and the conditional PGF of $N_2$ given it:
\begin{equation}
