...
\end{figure}
This parameter choice is motivated by two facts. First, class-$2$ still sees Poisson arrivals, so the probabilistic argument of Model-$A$ carries over. Second, substituting these values into the general fundamental equation~\eqref{eq:gen:fundamental_equation} leaves a first-order linear ODE in $x$: with no $y$-derivatives the boundary function $P_y(y)$ comes out of the integral, sparing us the Volterra equation that left Model-$B$ open (\S\ref{sec:model_b}).

\textbf{Stability.} Unlike the models without abandonment, stability here is non-trivial. The class-$1$ queue is an $M/M/1{+}M$ (Erlang-A) subsystem, which is positive recurrent for \emph{every} load (\S\ref{ssec:erlangA}); abandonment keeps the priority queue from ever building up, so class-$1$ on its own never destabilises the system. The only constraint comes from class-$2$, which---as the probabilistic argument of \S\ref{ssec:C2:prob} makes precise---sees an $M/G/1$ queue whose service time is one class-$1$ busy period $B_C$. That queue is stable exactly when its utilisation stays below one:
\begin{equation}
...
\end{equation}
the equivalence following from the mean busy period $\mathbb{E}[B_C]$ of~\eqref{eq:erlangA:bp_mean} together with the Kummer identity $\mu\bigl({}_1F_1(1;\tfrac{\mu}{\theta_1};\tfrac{\lambda_1}{\theta_1})-1\bigr)=\lambda_1\,{}_1F_1(1;\tfrac{\mu}{\theta_1}+1;\tfrac{\lambda_1}{\theta_1})$. We assume~\eqref{eq:C2:stability} throughout; under it the joint chain is positive recurrent and the stationary PGF $P(x,y)$ exists. The condition is strictly weaker than the no-abandonment requirement $\rho<1$ of Models~$A$, $B$ and~$B_2$: abandonment thins the priority queue, so class-$2$ can be stable even when $\rho\ge1$. The comparison and the sharpness of~\eqref{eq:C2:stability} are established in \S\ref{ssec:C2:prob}.

The following theorem states the result, obtained by both an analytical and a probabilistic route.
...
\begin{proof}
The proof runs in three steps, carried out in \S\S\ref{ssec:C2:anal}--\ref{ssec:C2:recovery}.

\emph{Step~(i): reduction to a first-order ODE in $x$.}  Substituting the Model-$C_2$ parameters into the general fundamental equation removes both $y$-derivatives and leaves, for each fixed $y$, a first-order linear ODE in $x$~\eqref{eq:C2:fundamental}.  Its integrating factor (\S\ref{sssec:if}) is
\[
...
We again take the viewpoint of a class-$2$ customer waiting while the server is busy. Unlike under jockeying, class-$1$ customers here truly leave the system, so the class-$2$ Poisson arrival stream is undisturbed. Because class-$1$ holds non-preemptive priority (\S\ref{ssec:priority}), a waiting class-$2$ customer is not served while any class-$1$ customer is present ($N_1>0$); its \emph{effective service time} is therefore one full busy period $B_C$ of the class-$1$ subsystem---now an $M/M/1{+}M$ queue with rates $\lambda_1,\mu,\theta_1$, rather than the pure $M/M/1$ of Model-$A$. By the \emph{memorylessness} of the exponential service, arrival and abandonment clocks these effective service times are i.i.d., so class-$2$ sees an $M/G/1$ queue with Poisson input $\lambda_2$ and service distributed as $B_C$. Its LST $\widetilde{B}_C(s)$ and mean $\mathbb{E}[B_C]$ come from the first-step analysis of \S\ref{ssec:erlangA} (Equations~\eqref{eq:erlangA:bp_lst}--\eqref{eq:erlangA:bp_mean}).

This $M/G/1$ is stable iff its utilisation $\lambda_2\,\mathbb{E}[B_C]$ is below one, the condition~\eqref{eq:C2:stability}. The condition is also necessary, not merely sufficient: when $\lambda_2\,\mathbb{E}[B_C]\ge1$ the embedded $M/G/1$ is null recurrent (at equality) or transient (strictly above) and its queue length has no stationary law; since the class-$2$ count is a deterministic function of the joint state $(n_1,n_2)$, a divergent class-$2$ queue rules out positive recurrence of the joint chain. We argue this at the level of the embedded $M/G/1$, whose marginal dynamics are exact, rather than through a Lyapunov function on the joint generator.

The condition is also strictly weaker than the no-abandonment requirement $\rho=\rho_1+\rho_2<1$, since for every $\theta_1>0$ the Kummer series is dominated term by term by the geometric series for $1/(1-\rho_1)$. Indeed, using $1^{(n)}=n!$, the series~\eqref{eq:kummer:series} reduces to
\[
...
\]
and since $\tfrac{\mu}{\theta_1}+k>\tfrac{\mu}{\theta_1}$ for every $k\ge1$, the $n$-th term is at most $(\lambda_1/\theta_1)^n/(\mu/\theta_1)^n=\rho_1^{\,n}$, strictly so for $n\ge1$. Summing, ${}_1F_1(1;\,\mu/\theta_1+1;\,\lambda_1/\theta_1)<\sum_{n\ge0}\rho_1^{\,n}=1/(1-\rho_1)$: abandonment in the priority queue stabilises the non-priority queue.

Applying the Pollaczek--Khinchine formula~\eqref{eq:pk:pgf} (\S\ref{ssec:pk}) with the identifications $\lambda\mapsto\lambda_2$, $\widetilde{B}\mapsto\widetilde{B}_C$, $\rho_B=\lambda_2\,\mathbb{E}[B_C]$ gives the PGF of the stationary number of class-$2$ customers in that $M/G/1$:
\begin{equation*}
    L_2(y) \;=\; \frac{(1 - \lambda_2 \mathbb{E}[B_C])\,(1-y)\,\widetilde{B}_C(\lambda_2(1-y))}{\widetilde{B}_C(\lambda_2(1-y)) - y}.
\end{equation*}
Because $\theta_2=\gamma_1=\gamma_2=0$, the class-$2$ arrivals are Poisson and lack anticipation, so by \emph{PASTA} (\S\ref{ssec:priority}) the time-stationary number of class-$2$ customers present has the $M/G/1$ steady-state law. Conditioning on $\{\text{busy},\,N_1=0\}$---the event on which a class-$2$ customer sits at the head of its queue---identifies that count with $N_2$:
\begin{equation}
...
\begin{equation*}
    P_y(y) \;=\; \pi(0,0)\,\frac{(1-y)\,\widetilde{B}_C(\lambda_2(1-y))}{\widetilde{B}_C(\lambda_2(1-y)) - y},
\end{equation*}
in agreement with~\eqref{eq:C2:Py}. This is the same expression as in the probabilistic argument for Model-$A$ (Equation~\eqref{eq:A:Py}); the only change is the effective service time, now the busy period of an $M/M/1{+}M$ subsystem rather than of a pure $M/M/1$.
...
Setting $x=y=z$ in~\eqref{eq:C2:fundamental}, the abandonment convection $\theta_1 xy(x-1)\,\partial_xP$ does \emph{not} vanish (the factor $(x-1)|_{x=z}\neq0$ for $z\in(0,1)$), and the diagonal equation reads
\begin{equation}
    ...
    \label{eq:C2:diag}
\end{equation}
The contrast with Model-$A$ is instructive. There the diagonal substitution also kills the boundary term $\mu(x-y)\,P_y(y)$, but with no convection at all the equation became a purely algebraic relation in $P(z,z)$, with closed form $P(z,z)=\pi(0,0)/(1-\rho z)$~\eqref{eq:A:Pzz}, from which $\mathbb{E}[N_1]+\mathbb{E}[N_2]$ followed by one differentiation. Here the surviving convection injects the off-diagonal slope $\partial_x P(z,z)$ as a \emph{second} unknown, and no companion equation closes the system; hence no closed form for $P(z,z)$---and so for the total $\mathbb{E}[N_1]+\mathbb{E}[N_2]$---is available. Abandonment removes customers and lowers both means relative to Model-$A$.

\subsubsection*{Class-2 mean queue length via \texorpdfstring{$P(1,y)$}{P(1,y)}}

Setting $x=1$ in~\eqref{eq:C2:fundamental}, the abandonment term vanishes and the coefficient of $P(1,y)$ collapses to $\lambda_2 y(1-y)$:
\begin{equation*}
