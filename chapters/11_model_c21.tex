...
\end{equation}
This is \emph{weaker} than $\rho<1$: if $\rho<1$ then $\mu-\lambda>0$ and the condition holds trivially, but it can also hold with $\rho\ge1$ provided $\theta_1$ is large enough. Moreover, Equation~\eqref{eq:C21:stability} implies $\rho_C:=\lambda_1/(\mu+\theta_1)<1$. Hence $\mu+\theta_1>\lambda_1$, i.e.\ $\rho_C<1$.
...
\begin{proof}
    ...
    $\theta_1\bigl(1-x^{-1}\bigr)\bigl[P(x,y)-P_y(y)\bigr]$; multiplying by $xy$ gives $\theta_1 y(x-1)\bigl[P(x,y)-P_y(y)\bigr]$. Adding to the Model-$A$ equation and rearranging yields Equation~\eqref{eq:C21:fundamental}.
\end{proof}
...
\begin{proof}[Proof of Corollary~\ref{cor:C21:pi0}]
    ...
    \begin{equation}
        ...
        \label{eq:C21:P1y}
    \end{equation}
    Letting $y\to1$ and using Equation~\eqref{eq:C21:P11} and the cut identity of Theorem~\ref{thm:gen:boundary} ($\pi(0,0)=\lambda\pi_0/\mu$, Equation~\eqref{eq:C21:idle}), we have
    \begin{equation*}
...
\begin{proof}[Proof of Theorem~\ref{thm:C21:PGF}]
    ...
    \end{equation*}
    So $x=1$ lies between the two roots and $x^*(y)<1<x^+(y)$ for $y\in[0,1)$; at $y=1$, $x^*(1)=1$ and $x^+(1)=(\mu+\theta_1)/\lambda_1>1$ (the latter by $\rho_C<1$). Hence $x^*(y)$ is the unique root inside the unit disc. Differentiating Equation~\eqref{eq:C21:xstar} at $y=1$,
    \begin{equation}
        \frac{d}{dy}x^*(1) = \frac{\lambda_2}{\mu+\theta_1-\lambda_1}.
        \label{eq:C21:xstar_deriv}
    \end{equation}
    
    Setting $x=x^*(y)$ in Equation~\eqref{eq:C21:fundamental} forces the RHS to vanish,
    \begin{equation*}
