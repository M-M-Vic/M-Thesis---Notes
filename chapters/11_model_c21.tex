...
simplification of the class-$1$ abandonment model Model-$C_2$. Where Model-$C_2$ lets every
waiting class-$1$ customer renege at the linear rate $\theta_1 n_1$ --- making the class-$1$
subsystem an $M/M/1{+}M$ (Erlang-A) queue and the fundamental equation an ODE --- here only
the \emph{customer at the head of Queue~1} abandons, at the fixed rate $\theta_1$ whenever
$n_1\ge1$. As in Model-$\BH$, the flat rate $\theta_1\,\mathbf{1}_{\{n_1\ge1\}}$ contributes
an \emph{algebraic} term rather than a derivative, and the Kernel Method applies directly.
The one essential difference is that abandonment is a \emph{true departure}: it removes a
customer and so breaks the conservation of the total count that Model-$\BH$ enjoyed, so the
empty-state probabilities and the class-$2$ mean queue length need separate arguments.
...
$\rho_C:=\lambda_1/(\mu+\theta_1)<1$,\footnote{Write the left-hand side
of~\eqref{eq:C21:stability} as $(\mu+\theta_1)(\mu-\lambda)+\lambda_1\theta_1
=\mu(\mu+\theta_1-\lambda)-\theta_1\lambda_2$. If $\mu+\theta_1\le\lambda_1$ then
$\mu+\theta_1-\lambda\le\lambda_1-\lambda=-\lambda_2<0$, so this expression is negative,
contradicting~\eqref{eq:C21:stability}. Hence $\mu+\theta_1>\lambda_1$, i.e.\ $\rho_C<1$.}
so the priority subqueue is itself stable whenever the condition holds.
...
\end{equation*}
so, by the same upward-parabola argument as in~\S\ref{sssec:B21:analytical}, $x=1$ lies
between the two roots and $x^*(y)<1<x^+(y)$ for $y\in[0,1)$; at $y=1$, $x^*(1)=1$ and
$x^+(1)=(\mu+\theta_1)/\lambda_1>1$ (the latter by $\rho_C<1$). Hence $x^*(y)$ is the unique
root inside the unit disk. Differentiating~\eqref{eq:C21:xstar} at $y=1$,
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:C21:Py}
\end{equation}
Write $\Delta(y):=(\mu+\theta_1 y)x^*(y)-y(\mu+\theta_1)$ for the denominator
of~\eqref{eq:C21:Py}; at $y=0$, $\Delta(0)=\mu\,x^*(0)$ (with $x^*(0)>0$), so
$P_y(0)=\pi(0,0)$, exactly as for Model-$\BH$ at~\eqref{eq:B21:Py}.

Substituting~\eqref{eq:C21:Py} into~\eqref{eq:C21:fundamental} and writing the kernel as
$-\lambda_1 y\,(x-x^*(y))(x-x^+(y))$, the right-hand side becomes
\begin{equation*}
...
\end{equation*}
Substituting $\Delta(y)$ into the curly bracket, the $x\,x^*(y)$ terms cancel and it
collapses to $(\mu+\theta_1)\,y\,(x-x^*(y))$ --- with no recourse to the kernel relation.
Dividing by the factored kernel cancels $(x-x^*(y))$, giving
\begin{equation*}
...
Models~$A$ and~$C_2$ does not transfer, and we keep this subsection only to record why. The
reason here is specific to abandonment: head abandonment fires only while a class-$1$
customer \emph{waits} ($N_1\ge1$) and is inactive while the single class-$1$ present is in
service, so the class-$1$ busy period is not that of any $M/M/1$ or $M/M/1{+}M$ queue.
Class~$2$ therefore sees a non-renewal $M/G/1$ input, no Pollaczek--Khinchine representation
of $P_y(y)$ exists, and the analytical route of~\S\ref{sssec:C21:analytical} is the operative
one.
