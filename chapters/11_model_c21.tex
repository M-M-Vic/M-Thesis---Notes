...
\begin{proof}[Proof of Corollary~\ref{cor:C21:pi0}]
    ...
    \begin{equation}
        ...
        \label{eq:C21:P1y}
    \end{equation}
    Letting $y\to1$ and using~\eqref{eq:C21:P11} and the cut identity of Theorem~\ref{thm:gen:boundary} ($\pi(0,0)=\lambda\pi_0/\mu$, Equation~\eqref{eq:C21:idle}), we have
    \begin{equation*}
    ...
    $\pi(0,0)=\lambda\pi_0/\mu$. The denominator $\mu(\mu+\theta_1)+\lambda_1\theta_1$ is strictly positive, so $\pi_0>0$ is equivalent to the stability condition~\eqref{eq:C21:stability}. In the renewal--reward form $\pi_0=1/\bigl(1+\lambda\,\mathbb{E}[B]\bigr)$ of Theorem~\ref{thm:gen:boundary}, this is a whole-system mean busy period $\mathbb{E}[B]=(\mu+\theta_1)/\bigl[(\mu+\theta_1)(\mu-\lambda)+\lambda_1\theta_1\bigr]$, which reduces to the $M/M/1$ value $(\mu-\lambda)^{-1}$ as $\theta_1\to0^+$.
\end{proof}
...
\begin{proof}[Proof of Theorem~\ref{thm:C21:PGF}]
    ...
    \end{equation*}
    So $x=1$ lies between the two roots and $x^*(y)<1<x^+(y)$ for $y\in[0,1)$; at $y=1$, $x^*(1)=1$ and $x^+(1)=(\mu+\theta_1)/\lambda_1>1$ (the latter by $\rho_C<1$). Hence $x^*(y)$ is the unique root inside the unit disc. Differentiating~\eqref{eq:C21:xstar} at $y=1$,
    \begin{equation}
