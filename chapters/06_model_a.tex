...
a not very accurate approximate result, for which an evaluation is provided in \ref{apx:A:PPGF_Stilde}.
...
\begin{theorem}\label{thm:A:PGF}
    ...
    independently of the class. Under the stability condition $\rho\leq 1$ ~\eqref{eq:A:stability}, the
...
\begin{corollary}\label{cor:A:pi0}
    Under the stability condition $\rho \leq 1$~\eqref{eq:A:stability}, the empty-state probabilities
    for Model-$A$ are
    \begin{equation}
...
\begin{proof}[Proof of Theorem~\ref{thm:A:PGF}, analytical approach.]
    ...
    \end{equation*}
    We apply the \emph{Kernel Method}, used in ~\cite{cohen1956delay}: fix $y\in(0,1]$ and find $x^*(y)$ such that the LHS of the expression above is zero. Since $P(x,y)$ is a PGF, it is bounded. At anysuch root the LHS vanishes, so the RHS must also vanish:
    \[
