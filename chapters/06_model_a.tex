...
\begin{proof}[Proof of Theorem~\ref{thm:A:PGF}, probabilistic approach.]
    This proof mirrors the analytical one, but derives the boundary function $P_y(y)$ by means of probabilistic arguments. We again begin from the fundamental equation for Model-$A$ (Equation~\eqref{eq:A:fundamental}):
    \begin{equation}
        \left[ (\lambda_1 + \lambda_2 + \mu) xy - \mu y - \lambda_1 x^2 y - \lambda_2 x y^2 \right] P(x,y) =
    ...
    into Equation~\eqref{eq:A:prob_starting}:
    \begin{align*}
        \begin{split}
            \left[ (\lambda_1+\lambda_2+\mu)xy - \mu y - \lambda_1 x^2y - \lambda_2 xy^2 \right] P(x,y)
            &= \mu(x-y) \frac{x^*(y)(1-y)}{x^*(y)-y} \rho(1-\rho) - \mu x(1-y) \rho(1-\rho)
...
\begin{proof}
    ...
    $\{\pi(0,n)\}$, and from Equation~\eqref{eq:A:Py} with $\pi(0,0)=\rho(1-\rho)$,
    \begin{equation*}
...
\begin{approximation}[Heuristic PPGF on $\widetilde{S}$]\label{apx:A:PPGF_Stilde}
    ...
    $\mu y^n(1-y)\,\tpi(n+1,n+1)$ in Equation~\eqref{eq:A:ppgf_Stilde_dynamics} ---which is
