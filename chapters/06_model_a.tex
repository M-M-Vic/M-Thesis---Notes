\section{Analysis of Model-\texorpdfstring{$A$}{A}}\label{sec:model_a}

Model-$A$ is the baseline model, obtained by setting all jockeying and abandonment
parameters to zero ($\gamma_1=\gamma_2=\theta_1=\theta_2=0$), leaving an ordinary
two-class $M/M/1$ queue with non-preemptive priority ---the classical, extensively
studied system of \S\ref{sec:literature}. We analyse it in both state spaces $S$ and
$\widetilde{S}$: on the former we give two complete proofs, on the latter
a conditionally accurate approximation, evaluated in Approximation~\ref{apx:A:PPGF_Stilde}.
The starting points of the analysis are the fundamental equations of
Lemma~\ref{lem:gen:fundamental_equation} for $S$ and Lemma~\ref{lem:gen:PPGF_dynamics}
for $\widetilde{S}$, specialised to these parameter values.
...
\begin{figure}[H]
    ...
    are present; the jockeying and abandonment rates are all zero.}
    \label{fig:model-a-queue}
\end{figure}

\subsection{Analysis in state space \texorpdfstring{$S$}{S}}
We begin from the fundamental equation of Lemma~\ref{lem:gen:fundamental_equation} with
$\gamma_1=\gamma_2=\theta_1=\theta_2=0$, in which the boundary function $P_y(y)=P(0,y)$ is
...
\begin{proof}[Proof of Corollary~\ref{cor:A:pi0}]
    Model-$A$ is the no-mechanism case $\gamma_1=\gamma_2=\theta_1=\theta_2=0$, so the statement is the specialisation of Corollary~\ref{cor:gen:no_abandon} ---equivalently Theorem~\ref{thm:gen:boundary} with the $M/M/1$ mean busy period $\mathbb{E}[BP]=(\mu-\lambda)^{-1}$, giving $\pi_0=1-\rho$ and $\pi(0,0)=\rho(1-\rho)$. The diagonal value $P(z,z)=\pi(0,0)/(1-\rho z)$ and the normalisation $P(1,1)=1-\pi_0=\rho$ are the reduction~\eqref{eq:gen:Pzz_diag} of the Model-$A$ kernel~\eqref{eq:A:fundamental} on $x=y=z$ (Corollary~\ref{cor:gen:diagonal}).
\end{proof}

\subsubsection{Analytical approach}\label{sssec:A:analytical}

\begin{proof}[Proof of Theorem~\ref{thm:A:PGF}, analytical approach.]
    We begin from the fundamental equation for Model-$A$~\eqref{eq:A:fundamental} and apply the \emph{Kernel Method}, used in~\cite{cohen1956delay}: for each fixed $y\in(0,1]$, we find $x^*(y)$ such that the coefficient of $P(x,y)$ ---the \emph{kernel}--- vanishes. Since $P(x,y)$ is a PGF, it is bounded, so at any such root the right-hand side of Equation~\eqref{eq:A:fundamental} must also vanish; this will determine $P_y(y)$ once the root is identified.
    ...
    We require our root to lie inside the closed unit disc for $y\in[0,1)$. Computing
    ...
    strictly inside the unit disc for $y\in[0,1)$ is $x^*(y)$:
    \begin{equation}
        x^*(y) = \frac{
    ...
    Equation~\eqref{eq:A:Py} into the fundamental equation~\eqref{eq:A:fundamental}, whose
    left-hand side stays unchanged while the right-hand side is simplified in stages:
    \begin{align*}
    ...
    \end{align*}
    Dividing both sides by $\mu$, so that the kernel is expressed in the loads $\rho_i$:
    \begin{equation*}
    ...
    yields $P(x,y)$ as in Theorem~\ref{thm:A:PGF} and concludes the proof.
\end{proof}


\subsubsection{Probabilistic approach}\label{sssec:A:probabilistic}
\begin{proof}[Proof of Theorem~\ref{thm:A:PGF}, probabilistic approach.]
    This proof mirrors the analytical one, but derives the boundary function $P_y(y)$ by means of probabilistic arguments; the starting point is again the fundamental equation for Model-$A$ (Equation~\eqref{eq:A:fundamental}).
    ...
    \begin{align*}
        ...
        \end{split}
    \end{align*}
    The identification of $L_2$ with the conditional law of $N_2$ given the event
    $\{\text{busy},\,N_1=0\}$ deserves a proof, and we give one by \emph{censoring}: watch
    the chain only while it is in the set $E=\{\text{busy},\,N_1=0\}$, splicing out the
    excursions away from $E$. The watched process is again a Markov chain, and its
    stationary law is exactly the conditional stationary law given $E$. While the system
    is in $E$, the class-$2$ queue grows at rate $\lambda_2$ and, when $N_2\ge1$, shrinks
    at rate $\mu$: no class-$1$ customer waits, so each service completion admits the head
    of Queue~2. The set $E$ is left in two ways, and neither disturbs $N_2$ on re-entry.
    From $(0,0)$ a completion empties the system; no customer waits while the server is
    idle, and the next arrival of either class enters service directly, so the chain
    re-enters $E$ with $N_2$ unchanged. From any state of $E$, a class-$1$ arrival (rate
    $\lambda_1$) opens a \emph{clearing spell}: by \emph{memorylessness} the residual
    service is $\mathrm{Exp}(\mu)$, so the waiting count $N_1$ performs a first passage
    from $1$ to $0$ of the birth--death chain with rates $(\lambda_1,\mu)$ ---one class-$1$
    busy period $B$--- during which the priority rule admits no class-$2$ customer into
    service and the class-$2$ queue accumulates a batch of $\mathrm{Poisson}(\lambda_2 B)$
    arrivals. Successive spells are i.i.d.\ by the strong Markov property. Watched on $E$,
    the class-$2$ queue is therefore a birth--death chain with an additional batch stream:
    single arrivals at rate $\lambda_2$, single departures at rate $\mu$, and, at rate
    $\lambda_1$, batches of size $K$ with
    $\mathbb{E}[y^{K}]=\mathbb{E}[e^{-\lambda_2(1-y)B}]=\widetilde{B}(\lambda_2(1-y))=x^*(y)$.
    Writing $G(y)$ for the stationary PGF of the watched chain and $p_0=G(0)$, balance of
    probability flow gives
    \begin{equation*}
        \mu\,\frac{1-y}{y}\,\bigl(G(y)-p_0\bigr)
        \;=\;\bigl[\lambda_2(1-y)+\lambda_1\bigl(1-x^*(y)\bigr)\bigr]\,G(y),
    \end{equation*}
    so that
    \begin{equation*}
        G(y)\;=\;\frac{\mu(1-y)\,p_0}{\mu(1-y)-\lambda_2\,y(1-y)-\lambda_1\,y\bigl(1-x^*(y)\bigr)}.
    \end{equation*}
    Since $x^*(y)$ is a root of the kernel quadratic
    $\lambda_1x^2-\left[\mu+\lambda_1+\lambda_2(1-y)\right]x+\mu=0$
    (\S\ref{sssec:A:analytical}), the quadratic rearranged at its root factorises as
    $\bigl(1-x^*(y)\bigr)\bigl(\mu-\lambda_1x^*(y)\bigr)=\lambda_2\,x^*(y)\,(1-y)$. This
    identity reduces the denominator above, giving
    $G(y)=p_0\bigl(\mu-\lambda_1x^*(y)\bigr)\big/\bigl(\mu-\lambda_1x^*(y)-\lambda_2y\bigr)$;
    the normalisation $G(1)=1$ (with $x^*(1)=1$) fixes $p_0=(1-\rho)/(1-\rho_1)$, and a
    second application of the same identity turns the expression into
    \begin{equation*}
        G(y)\;=\;\frac{1-\rho}{1-\rho_1}\cdot\frac{x^*(y)\,(1-y)}{x^*(y)-y}\;=\;L_2(y),
    \end{equation*}
    exactly the Pollaczek--Khinchine law built above. Hence
    $\mathbb{E}[y^{N_2}\mid\text{busy},N_1=0]=L_2(y)$: the clearing spells are where the
    busy period $B$ enters, and they make the $M/G/1$ reading of the construction precise.

    By the law of total expectation,
    \begin{equation*}
        P_y(y)=\mathbb{P}(\text{busy},N_1=0)\cdot\mathbb{E}[y^{N_2}\mid\text{busy},N_1=0] = \mathbb{P}(\text{busy},N_1=0)\cdot L_2(y).
    \end{equation*}
    The factor $\mathbb{P}(\text{busy},N_1=0)$ follows from a cut argument that makes
    precise the sense in which the priority queue operates autonomously. Only two kinds of
    transition change $N_1$. A class-$1$ arrival while the server is busy raises it by one,
    at rate $\lambda_1$ (an arrival finding the server idle enters service directly and
    leaves $N_1=0$); a service completion while $N_1\ge1$ lowers it by one, at rate $\mu$,
    since by \emph{memorylessness} every service in progress ---of either class--- completes
    at rate $\mu$, and the freed server then admits the head of Queue~1. Transitions between
    idle and busy occur only while $N_1=0$ and cross no level. Balance across the cut
    between $\{N_1\le k\}$ and $\{N_1\ge k+1\}$ therefore reads
    \begin{equation*}
        \lambda_1\,\mathbb{P}(\text{busy},\,N_1=k)\;=\;\mu\,\mathbb{P}(N_1=k+1),
        \qquad k\ge0,
    \end{equation*}
    and since $N_1\ge1$ implies a busy server, the recursion is geometric,
    \begin{equation*}
        \mathbb{P}(\text{busy},\,N_1=k)\;=\;\rho_1^{\,k}\,\mathbb{P}(\text{busy},\,N_1=0).
    \end{equation*}
    Summing over $k\ge0$ gives $\mathbb{P}(N_1=0\mid\text{busy})=1-\rho_1$ ---during busy time the
    priority queue exhibits exactly the geometric law of the ordinary $M/M/1$ queue with
    rates $(\lambda_1,\mu)$. Combined with $\mathbb{P}(\text{busy})=\rho$, we have
    \begin{equation*}
    ...
    Substituting Equation~\eqref{eq:A:Py_prob} and $\pi(0,0)=\rho(1-\rho)$ (Corollary~\ref{cor:A:pi0})
    into the fundamental equation~\eqref{eq:A:fundamental}, and simplifying the right-hand
    side in stages as in the analytical proof:
    \begin{align*}
            \left[ (\lambda_1+\lambda_2+\mu)xy - \mu y - \lambda_1 x^2y - \lambda_2 xy^2 \right] P(x,y)
            &= \mu(x-y) \frac{x^*(y)(1-y)}{x^*(y)-y} \rho(1-\rho) - \mu x(1-y) \rho(1-\rho) \\
            &= \mu(1-y) \left[ \frac{x^*(y)(x-y)}{x^*(y)-y} - x \right] \rho(1-\rho) \\
    ...
    \end{equation*}
    Multiplying both sides by $\left[ (1+\rho_1+\rho_2)xy - y - \rho_1 x^2y - \rho_2 xy^2 \right]^{-1}$
    to isolate $P(x,y)$ yields Theorem~\ref{thm:A:PGF} and concludes the proof.
\end{proof}


\subsubsection{Partial-PGF approach}

This proof follows Cohen~\cite{cohen1956delay}'s method. We keep the class-$1$
index $n_1$ as a discrete parameter and take transforms only with respect
to $N_2$, via the PPGF $\widetilde{P}(n_1,y)=\sum_{n_2\geq0}\pi(n_1,n_2)\,y^{n_2}$
from Equation~\eqref{eq:gen:ppgf_n1}. Multiplying the balance equations by $y^{n_2}$ and
...
\begin{proof}
    ...
    \end{align}
    Now we take transforms by summing all over their states, but only with respect to $n_2$, and we substitute our PPGF $\widetilde{P}(n_1,y)=\sum_{n_2=0}^{\infty} \pi(n_1,n_2)y^{n_2}$:
    \begin{align}
        \begin{split}
        ...
        \end{split} \nonumber \\
        \intertext{where $LHS_1$ and $RHS_j$ denote the terms of the display above, in the order written. Evaluating each summand:}
        \begin{split}
            & LHS_1 \\
        ...
        \end{split} \nonumber \\
        \intertext{Combining the terms above:}
        \begin{split}
            ...
            & \quad \quad + \; \lambda_2 y\,\widetilde{P}(n_1,y)
        \end{split} \nonumber \\
        \intertext{Rearranging, with the transforms on the left and the boundary probabilities on the right:}
        \begin{split}
    ...
    \end{align}
    By the $x$-boundary balance equation~\eqref{eq:A:ppgf_xbound}, the right-hand side of Equation~\eqref{eq:A:ppgf_final_boundary} vanishes completely, giving the homogeneous recurrence
    \begin{equation}
    ...
    \end{equation*}
    the latter making the geometric sequence $[\widetilde{x}^{+}(y)]^{n_1}$ non-summable. We therefore define
    \begin{equation}
    ...
    Together with $P_y(y)$, the geometric law~\eqref{eq:A:ppgf_geometric} already characterises the full joint distribution; what remains is to verify that it reproduces the closed form of Theorem~\ref{thm:A:PGF}.
    ...
    \end{equation}
    where $x^*(y)$ and $x^{+}(y)$ are the roots found in \S\ref{sssec:A:analytical}. By Vieta's product formula, $x^*(y)\,x^{+}(y)=\mu/\lambda_1=1/\rho_1$. This, together with $\widetilde{x}^*(y)$ in Equation~\eqref{eq:A:xtildestar}, gives $\widetilde{x}^*(y)=\rho_1 x^*(y)=1/x^{+}(y)$.
...
\subsection{The partial-PGF recursion on \texorpdfstring{$\widetilde{S}$}{S~}: scope and limitations}\label{ssec:A:Stilde}

Setting $\gamma_1=\gamma_2=\theta_1=\theta_2=0$ in the fundamental $\widetilde{S}$
equation of Lemma~\ref{lem:gen:PPGF_dynamics} ---equivalently, transforming the
$\widetilde{S}$ balance
equations~\eqref{eq:gen:tilde_balance_interior}--\eqref{eq:gen:tilde_balance_idle}
with the PPGF $\widetilde{P}(y,n)=\sum_{n_2=0}^n \widetilde{\pi}(n_2,n)\,y^{n_2}$---
...
The forcing term in Equation~\eqref{eq:A:ppgf_Stilde_dynamics} is controlled by the diagonal
...
\begin{proof}
    ...
    the coefficients $\pi(0,n)$, and from Equation~\eqref{eq:A:Py} with $\pi(0,0)=\rho(1-\rho)$,
    \begin{equation*}
...
\begin{approximation}[Heuristic PPGF on $\widetilde{S}$]\label{apx:A:PPGF_Stilde}
    ...
    $\mu y^n(1-y)\,\tpi(n+1,n+1)$ in Equation~\eqref{eq:A:ppgf_Stilde_dynamics} ---which
    decays geometrically in $n$ by Lemma~\ref{lem:A:diag_decay}--- the
    ...
    the steady-state marginal of the $M/M/1$ queue with rates $\lambda$ and $\mu$ for $n$ customers waiting.
\end{approximation}
...
forcing term is therefore exponentially small \emph{as $n$ grows} ---the recurrence
holds for every $n$, and for small $n$ the term need not be negligible--- and
neglecting it throughout yields an approximation whose accuracy is quantified below.
The resulting homogeneous recurrence is
\begin{equation*}
...
\end{equation*}
Note that $\widetilde{y}^*(y)$ differs from the kernel roots $x^*(y)$ and $\widetilde{x}^*(y)$ of the $S$-space analyses: there the transform variable enters through the linear coefficient $\lambda_2(1-y)$ of the characteristic polynomial, here through its constant term $\lambda_1+\lambda_2 y$.
...
\begin{equation*}
    \tP(1,n)= \rho(1-\rho) \cdot\rho^n= (1-\rho) \rho^{n+1},
\end{equation*}
which matches the stationary probability of $n+1$ customers in the $M/M/1$ queue
with rates $\lambda$ and $\mu$~\cite{adan2002queueing}.
\end{proof}
...
and solve Equation~\eqref{eq:A:ppgf_Stilde_dynamics} by variation of constants in $n$, which
