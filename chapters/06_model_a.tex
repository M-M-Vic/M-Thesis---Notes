...
all waiting customers are class~$2$; using $\tpi(n,n)=\pi(0,n)$ and the rate just proved
...
approximation is therefore accurate exactly when the forcing rate is the faster of the two,
$\rho<\widetilde{y}^*(y)$ on the relevant range of $y$ --- which holds when the priority
class carries most of the load, i.e.\ when $\rho_1/\rho$ is large and $\rho$ moderate, and
this is precisely the empirical validity region mapped in Figure~\ref{fig:val_ppgf}. The
obstruction is structural and recurs throughout this thesis: an unknown boundary/diagonal
...
(\S\ref{sec:model_b}, \S\ref{sec:model_x_anal}). A rigorous closure would retain the forcing
