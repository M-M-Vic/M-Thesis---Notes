...
Since jockeying and abandonment add state-dependent rates to the balance equations, the derivations require solving first-order linear ODEs and PDEs for the PGF. The three methods below are each invoked at specific steps of the analysis.
...
\emph{Variation of parameters} —also called \emph{variation of constants}— provides an alternative derivation of~\eqref{eq:ode:general} that makes the role of the free function explicit. The homogeneous equation $\frac{dP}{dx} + p(x;\,y)\,P = 0$ has general solution $P_h(x;y) = C/\mu_I(x;y)$ for an arbitrary constant $C$. To solve the full equation, one \emph{varies} that constant, setting $P(x,y) = C(x;\,y)/\mu_I(x;\,y)$ and substituting into~\eqref{eq:ode:standard}. Since the homogeneous part cancels, one obtains
\begin{equation}
    \frac{dC}{dx}(x;\,y) \;=\; q(x;\,y)\,\mu_I(x;\,y),
    \label{eq:ode:vop}
\end{equation}
and integrating recovers~\eqref{eq:ode:general} with $C_0(y) = C(0;\,y) = P(0,y)$.

In the PGF derivations, $C_0(y)$ is identified with the boundary generating function $P_y(y) = P(0,y)$. Its determination —either by direct probabilistic arguments or by demanding that $P(x,y)$ be analytic and bounded for $|x|\leq 1$— is the central analytical step in each model's solution.
