...
\end{corollary}
As in Model-$B$, the proof follows trivially by setting $x=y=z$ in our fundamental equation (Equation~\eqref{eq:B2:fundamental}). The surviving equation is identical to the homologous one in Model-$A$, so Corollary~\ref{cor:B2:pi00} is the no-abandonment instance of Theorem~\ref{thm:gen:boundary}, with the $M/M/1$ mean busy period $\mathbb{E}[B]=(\mu-\lambda)^{-1}$ (Corollary~\ref{cor:gen:no_abandon}).
...
\end{equation*}
where the roots $x^*(y),x^+(y)$ are those found for the kernel in Model-$A$ (Equation~\eqref{eq:A:xstar}), with $\lambda_1 x^* x^+ = \mu$ following from Vieta's formula. The Partial Fraction Decomposition of $p = N(x)/\bigl[\gamma_1 x(x-y)\bigr]$ reads as follows:
\begin{align*}
    A &= -\lambda_1, \nonumber\\
...
On the diagonal the value is known outright. Setting $x=y=z$ in the fundamental equation~\eqref{eq:B2:fundamental} drops the jockeying and boundary terms (each containing a factor $x-y$) and leaves Model~$A$'s balance, so
\begin{equation}
...
Substituting Equation~\eqref{eq:B2:Py} into the real-form expression~\eqref{eq:B2:Pxy_real}
