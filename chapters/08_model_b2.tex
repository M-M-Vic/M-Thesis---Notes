...
\end{equation}
Hence the prefactor $(y-x)^{-\beta}=(y-x)^{|\beta|}$ in~\eqref{eq:B2:Pxy_real} \emph{vanishes} as $x\to y^{-}$, while $\mathcal{H}_1,\mathcal{H}_2$ may \emph{diverge}: the product is an indeterminate $0\cdot\infty$ form. The resolution is the organizing idea of this step. A PGF must always be bounded, also in the interior point $x=y$; and we can use this condition to determine $P_y(y)$
...
\end{equation}
which is exactly the diagonal value $P(z,z)=\rho(1-\rho)/(1-\rho z)$ at $z=y$ --- the same value
Model~$A$ returns~\eqref{eq:A:Pzz}, obtained there with no extra input by the diagonal
cancellation. The general local solution adds a multiple of the homogeneous mode
$(x-y)^{-\beta}=(x-y)^{|\beta|}$, a non-integer power of $(x-y)$ ($|\beta|\notin\mathbb{Z}$
generically). But for fixed $y$ the PGF $P(\cdot,y)$ is a power series in $x$, convergent for
$|x|<1$ and hence smooth at the interior point $x=y<1$, whereas a non-integer power of $(x-y)$
is not; its coefficient must therefore vanish. It remains only to locate that coefficient
inside the explicit solution~\eqref{eq:B2:Pxy_real}.
...
\begin{align*}
    \text{non-integer power }(y-x)^{|\beta|}:&\quad
...
\end{align*}
The second reproduces the regular value~\eqref{eq:B2:regular_value} and the third is smooth
at $x=y$; only the first is a non-integer power ($|\beta|\notin\mathbb{Z}$), hence not smooth
there. As $P(\cdot,y)$ is a power series in $x$, and so smooth at $x=y$, its bracket must
vanish,
\begin{equation}
