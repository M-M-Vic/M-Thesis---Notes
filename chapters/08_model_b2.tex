...
\end{equation}
Hence the prefactor $(y-x)^{-\beta}=(y-x)^{|\beta|}$ in~\eqref{eq:B2:Pxy_real} \emph{vanishes} as $x\to y^{-}$, while $\mathcal{H}_1,\mathcal{H}_2$ may \emph{diverge}: the product is an indeterminate $0\cdot\infty$ form, whose resolution fixes $P_y(y)$.
...
\begin{align*}
    ...
    \text{singular part of }\mathcal{H}_2:&\quad
        -\Psi(y)(1-y)\pi(0,0)\,\frac{g_2(y)}{\beta} = \frac{\mu\,\pi(0,0)}{\mu-\lambda y}, \\
