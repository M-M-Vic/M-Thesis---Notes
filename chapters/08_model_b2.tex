...
Model-$B_2$ keeps one-way jockeying ($\gamma_1>0$, $\gamma_2=0$) and no abandonment
($\theta_1=\theta_2=0$); it is the specialisation of Model-$B$ in which only the $1\to2$
transfer survives, and this single restriction is what makes it solvable. Setting $\gamma_2=0$
removes the $\partial P/\partial y$ term from the fundamental equation, so the Method of
Characteristics and variation of parameters that Model-$B$ required (\S\ref{sssec:moc},
\S\ref{sssec:vop}) are no longer needed: for each fixed $y\in(0,1]$ the equation is an
ordinary first-order linear ODE in $x$, solved by a single integrating factor
(\S\ref{sssec:if}). The resulting solution carries a branch singularity at the interior point
$x=y$ with a negative exponent, and demanding that $P(\cdot,y)$ be holomorphic there pins the
boundary function $P_y(y)$. This interior-analyticity condition is the $B_2$ counterpart of
the boundary-boundedness argument used for Model-$C_2$.
...
\begin{theorem}\label{thm:B2:PGF}
    ...
    across the interior point $x=y$ is the content of the no-branch condition established in
...
Fix $y\in(0,1]$ and divide the reduced fundamental equation~\eqref{eq:B2:fundamental} by the
coefficient $\gamma_1 xy(x-y)$ of $\partial P/\partial x$. The factor $y$ cancels against the
overall $y$ in the bracket multiplying $P$, and the equation takes the standard first-order
form $\tfrac{dP}{dx}+p(x;y)\,P=q(x;y)$ of \S\ref{sssec:if}
(Equation~\eqref{eq:ode:standard}), with
\begin{align}
...
\begin{equation}
    ...
    \label{eq:B2:pfrac}
\end{equation}
We work on the principal triangle $0\le x\le y$, where $x-y\le0$; the region $y<x\le1$ is
recovered at the end by analytic continuation. On the triangle the antiderivative of the
simple-pole term is $\beta\ln|x-y|=\beta\ln(y-x)$, so integrating the partial
fraction~\eqref{eq:B2:pfrac} term by term gives, up to a constant,
\begin{align*}
    \int^{x}\! p(t;y)\,dt
...
\end{align*}
with the exponents $\alpha(y)=\mu/(\gamma_1 y)>0$ and
$\beta(y)=(1-y)(\lambda y-\mu)/(\gamma_1 y)$ of~\eqref{eq:B2:exponents_thm}. Exponentiating,
and dropping the multiplicative constant to which an integrating factor is indifferent, gives
\begin{equation*}
    \mu_I(x;y) = e^{-\lambda_1 x/\gamma_1}\,x^{\,\alpha(y)}\,(y-x)^{\,\beta(y)},
\end{equation*}
real-valued on the triangle because we take the base $(y-x)>0$ rather than $(x-y)$; its
reciprocal $1/\mu_I$ is the homogeneous solution, playing here the role that $P_h$ played for
Model-$B$ (\S\ref{sssec:vop}). It indeed solves $\mu_I'/\mu_I=p$:
\begin{align*}
    ...
    &= -\frac{\lambda_1}{\gamma_1} + \frac{\alpha}{x} + \frac{\beta}{x-y}
     = p(x;y).
\end{align*}
The general solution of \S\ref{sssec:if} (Equation~\eqref{eq:ode:general}) now applies, and its
arbitrary function is fixed immediately: since $\alpha>0$ we have $\mu_I\to0$ as
$x\to0^{+}$, so the boundary contribution $P(0,y)\,\mu_I(0;y)$ vanishes and integrating
$\tfrac{d}{dx}[P\,\mu_I]=q\,\mu_I$ over $(0,x]$ leaves
\begin{equation*}
    P(x,y)\,\mu_I(x;y) = \int_0^x q(t;y)\,\mu_I(t;y)\,dt.
\end{equation*}
Multiplying $q$ from~\eqref{eq:B2:qxy} by $\mu_I$ and applying the identity
$(y-t)^{\beta}/(t-y)=-(y-t)^{\beta-1}$ to the second term, the integrand separates into two
real pieces,
\begin{equation*}
...
% ─────────────────────────────────────────────────────────────────
\medskip\noindent\textit{Step~(ii): Determination of $P_y(y)$ by analyticity at $x=y$.}
\medskip

The factor $(x-y)$ in the $x$-convection coefficient places the operative singularity at the
interior point $x=y$, the analyticity-pinning regime anticipated in
Remark~\ref{rem:gen:reading}. No probabilistic (Pollaczek--Khinchine) route is available, in
contrast with Models~$A$ and~$C_2$: one-way jockeying makes the service a class-2 customer
experiences depend on the class-1 contents at the (random) jockeying instants, so the class-2
input is not a Poisson stream feeding an $M/G/1$ subsystem and no busy-period reduction applies
(cf.\ the discussion of Model-$B$ in \S\ref{sec:model_b}). We therefore fix $P_y(y)$
analytically.

Under stability $\mu-\lambda y>0$ for $y\in(0,1)$, so the second exponent is negative,
\begin{equation}
    \beta(y) = \frac{(1-y)(\lambda y-\mu)}{\gamma_1 y} < 0,
    \qquad 0<y<1.
    \label{eq:B2:beta_negative}
\end{equation}
This sign is the crux of the $B$--$B_2$ parallel: Model-$B$ carries the same diagonal factor
with exponent $E(x,y)>0$ (\S\ref{sec:model_b}, Equation~\eqref{eq:B:E}), which merely vanishes
on the diagonal, whereas the one-way restriction flips the exponent to $\beta<0$, turning that
factor into the branch term the analyticity condition must remove.
Hence the prefactor $(y-x)^{-\beta}=(y-x)^{|\beta|}$ in~\eqref{eq:B2:Pxy_real} \emph{vanishes}
as $x\to y^{-}$, while $\mathcal{H}_1,\mathcal{H}_2$ may \emph{diverge} there: the product is an
indeterminate $0\cdot\infty$ form. The resolution is the organising idea of this step. A
probability generating function is holomorphic on the open unit disc, so $P(\cdot,y)$ must be
analytic at the interior point $x=y$; but the general solution carries a non-integer power
$(y-x)^{|\beta|}$, a branch term that vanishes at $x=y$ yet is not analytic there. Holomorphy
therefore forces the coefficient of that branch term to zero, and this single condition pins
$P_y(y)$.
...
\end{equation}
consistent with the diagonal value~\eqref{eq:B2:Pxx}. The general local solution adds a
multiple of the homogeneous mode $(x-y)^{-\beta}=(x-y)^{|\beta|}$ --- the branch term
($|\beta|\notin\mathbb{Z}$ generically) whose coefficient holomorphy forces to zero. It
remains only to locate that coefficient inside the explicit solution~\eqref{eq:B2:Pxy_real}.
...
\end{align*}
where $\mathrm{f.p.}$ denotes the finite part (the $x\to y$ limit of the integral once its
singular term has been subtracted) and the omitted terms are of higher order. Multiplying by
$\Psi(x)(y-x)^{-\beta}$, and using $\Psi(y)\,g_2(y)=\mu/(\gamma_1 y)$, sorts $P(x,y)$ near
$x=y$ into three contributions, one per power of $(y-x)$:
\begin{align*}
    \text{branch part }(y-x)^{|\beta|}:&\quad
        \Psi(x)(y-x)^{-\beta}\bigl[P_y(y)\,\mathrm{f.p.}\,\mathcal{H}_1
            + (1-y)\pi(0,0)\,\mathrm{f.p.}\,\mathcal{H}_2\bigr], \\
    \text{singular part of }\mathcal{H}_2:&\quad
        -\Psi(y)(1-y)\pi(0,0)\,\frac{g_2(y)}{\beta} = -\frac{D}{\beta} = P_*, \\
    \text{singular part of }\mathcal{H}_1:&\quad
        -\Psi(x)\,P_y(y)\,\frac{g_1(y)}{\beta+1}\,(y-x) \;\propto\; (y-x).
\end{align*}
The second reproduces the regular value~\eqref{eq:B2:regular_value} and the third is analytic
at $x=y$; only the first is non-analytic ($|\beta|\notin\mathbb{Z}$). Holomorphy of
$P(\cdot,y)$ at $x=y$ therefore forces its bracket to vanish,
\begin{equation}
...
representation is read as its analytic continuation in $b$ past the convergence range $b>0$ ---
which is precisely the finite part --- and gives
\begin{align}
