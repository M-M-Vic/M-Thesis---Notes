...
\begin{proof}
    Setting $x=y=z$ in the fundamental equation~\eqref{eq:gen:fundamental_equation} with
    $\theta_1=\theta_2=0$, both jockeying terms carry the factor $(x-y)$ and vanish, dragging
    the boundary term $\mu(x-y)P_y(y)$ with it. The surviving diagonal equation is therefore
    \emph{identical} to that of Model-$A$, and reduces as in the proof of
    Corollary~\ref{cor:A:pi0} (with $\lambda=\lambda_1+\lambda_2$) to
    \begin{equation}
        P(z,z) = \frac{\mu\,\pi(0,0)}{\mu-\lambda z} = \frac{\pi(0,0)}{1-\rho z}.
        \label{eq:B:Pxx}
    \end{equation}
    The empty-state probabilities $\pi_0=1-\rho$ and $\pi(0,0)=\rho(1-\rho)$ then follow by
    the same global-balance and normalisation argument as in Corollary~\ref{cor:A:pi0}:
    jockeying conserves the total-customer process and cannot change the marginal idle
    probability. The same identities therefore hold in the other no-abandonment model,
    Model-$B_2$; Model-$C_2$ retains the universal idle balance
    $\pi(0,0)=\rho\pi_0$~\eqref{eq:gen:pi00_general} but, with abandonment present, attains
    $\pi_0=1-\rho$ only in the limit $\theta_1\to0^+$.
\end{proof}
