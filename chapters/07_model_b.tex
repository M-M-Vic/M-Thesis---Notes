...
\begin{proof}[Proof of Claim~\ref{clm:B:volterra}]
    ...
    This step upgrades the homogeneous solution $P_h$ of~\eqref{eq:B:Ph} to the full
    solution along each characteristic. It needs two ingredients. The first is the
    polynomial $A$ of~\eqref{eq:B:Adef} from Step~(i), through which the algebraic
    bracket of~\eqref{eq:B:fundamental} factorises as $y\,A(x,y)$. The second is the
    right-hand side of~\eqref{eq:B:fundamental}, which we name
    \begin{equation}
        G(x,y) := \mu(x-y)\,P_y(y) - \mu\,x(1-y)\,\pi(0,0);
        \label{eq:B:Gdef}
    \end{equation}
    this forcing carries the unknown boundary trace $P_y(y)=P(0,y)$ together with the
    known constant $\pi(0,0)=\rho(1-\rho)$ of Corollary~\ref{cor:B:pi00}.

    With these names the Model-$B$ fundamental equation~\eqref{eq:B:fundamental} is
    ...
    Hence, along the characteristic $\gamma_2 x+\gamma_1 y=C_1$ found in Step~(i)
    ...
    \begin{equation}
        ...
        \label{eq:B:ode_char}
    \end{equation}
    Its homogeneous part $\frac{dP}{dx}=f(x;C_1)\,P$ is exactly the equation integrated
    in Step~(i), so $P_h$ is its fundamental solution and the coefficient $f$ need not be
    written out; its forcing is the right-hand side divided by the coefficient of
    $\partial_x P$,
    \begin{equation}
        h(x;C_1)=\frac{G\bigl(x,y(x)\bigr)}{\gamma_1\,x\,y(x)\,\bigl(x-y(x)\bigr)}.
        \label{eq:B:hdef}
    \end{equation}
    Equation~\eqref{eq:B:ode_char} is thus an instance of~\eqref{eq:ode:vop_standard} with
    the known homogeneous solution $P_h$, so the variation-of-parameters
    formula~\eqref{eq:ode:vop_general} of \S\ref{sssec:vop} delivers its general solution
    at once, the arbitrary constant becoming a function $F(C_1)$ of the characteristic
    label:
    \begin{equation}
