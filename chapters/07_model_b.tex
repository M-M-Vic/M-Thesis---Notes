...
\begin{proof}[Proof of Claim~\ref{clm:B:volterra}]
    ...
    Recall $\lambda = \lambda_1+\lambda_2$. Along any fixed characteristic
    $C_1 = \gamma_2 x + \gamma_1 y$ is constant, so the exponent~\eqref{eq:B:E}
    depends only on $u := C_1$, which ranges over $(0,\gamma_1+\gamma_2)$ as
    $(x,y)$ runs through $(0,1)^2$:
    \begin{equation*}
        E(u) = \frac{\lambda}{(\gamma_1+\gamma_2)^2}\,u - \frac{\lambda+\mu}{\gamma_1+\gamma_2} + \frac{\mu}{u}.
    \end{equation*}
    It suffices to show $E(u)>0$ on $(0,\gamma_1+\gamma_2)$, which follows at once
    from two facts. First, $E$ vanishes at the right endpoint,
    \begin{equation*}
        E(\gamma_1+\gamma_2)
        = \frac{\lambda-(\lambda+\mu)+\mu}{\gamma_1+\gamma_2} = 0.
    \end{equation*}
    Second, $E$ is strictly decreasing on $(0,\gamma_1+\gamma_2]$:
    \begin{align*}
        \begin{split}
        ...
        \end{split}
    \end{align*}
    where the middle step uses $u\le\gamma_1+\gamma_2$ and the last uses
    $\mu>\lambda$ (i.e.\ $\rho<1$). A function that decreases strictly to $0$ at
    its right endpoint is positive to the left of it, so
    \begin{equation*}
        E(u) > E(\gamma_1+\gamma_2) = 0 \qquad\text{for all } u\in(0,\gamma_1+\gamma_2),
    \end{equation*}
    that is, $E(x,y) > 0$ for all $(x,y)\in(0,1)^2$. In particular $E\to0$ as
    $u\to(\gamma_1+\gamma_2)^-$ (i.e.\ as $(x,y)\to(1,1)$), while the $\mu/u$ term
    forces $E\to+\infty$ as $u\to0^+$; hence $E$ is bounded and bounded away from
    zero on every compact subset of $(0,1)^2$, though not uniformly on all of it.
