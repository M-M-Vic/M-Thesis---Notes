...
\begin{proof}[Proof of Claim~\ref{clm:B:volterra}]
    ...
    We now restore the right-hand side of the Model-$B$ fundamental equation~\eqref{eq:B:fundamental},
    whose algebraic coefficient is $y\,A(x,y)$ with $A$ from~\eqref{eq:B:Adef}. Along
    the characteristic $y(x)=(C_1-\gamma_2 x)/\gamma_1$ we have $dy/dx=-\gamma_2/\gamma_1$,
    so $\frac{d}{dx}P(x,y(x))=\partial_x P-\frac{\gamma_2}{\gamma_1}\partial_y P$ and the two
    jockeying terms collapse into a single total derivative:
    \begin{align*}
        \begin{split}
            \gamma_1 xy(x-y)\,\partial_x P + \gamma_2 xy(y-x)\,\partial_y P
            &= \gamma_1 xy(x-y)\Bigl(\partial_x P - \tfrac{\gamma_2}{\gamma_1}\partial_y P\Bigr)
        \end{split}\\
        \begin{split}
            &= \gamma_1 xy(x-y)\,\frac{d}{dx}P\bigl(x,y(x)\bigr).
        \end{split}
    \end{align*}
    The fundamental equation along the characteristic is therefore
    $y\,A(x,y)\,P+\gamma_1 xy(x-y)\frac{dP}{dx}=G(x,y)$, and dividing by
    $\gamma_1 xy(x-y)$ reduces it, exactly as in \S\ref{sssec:moc}, to a first-order
    linear ODE in $x$ at fixed $C_1$,
    \begin{equation}
        \frac{dP}{dx} = f(x;C_1)\,P + h(x;C_1).
        \label{eq:B:ode_char}
    \end{equation}
    Its homogeneous part $\frac{dP}{dx}=f(x;C_1)\,P$ is precisely the equation
    integrated in Step~(i), so $P_h$ of~\eqref{eq:B:Ph} is its fundamental solution and
    $f$ need never be written out. The forcing term carries the right-hand side,
    \begin{equation}
        ...
        \label{eq:B:hdef}
    \end{equation}
    so $h$ depends linearly on the unknown boundary trace $P_y(y)=P(0,y)$ and on the
    known constant $\pi(0,0)=\rho(1-\rho)$ of Corollary~\ref{cor:B:pi00}.
    Equation~\eqref{eq:B:ode_char} now has exactly the form~\eqref{eq:ode:vop_standard}
    with a known homogeneous solution $P_h$, so the variation-of-parameters
    formula~\eqref{eq:ode:vop_general} of \S\ref{sssec:vop} gives its general solution
    immediately, with the arbitrary constant promoted to the function $F(C_1)$ of the
    ...
    inhomogeneity $G(t,y(t))$ into the forcing $h$ of~\eqref{eq:B:hdef}:
    \begin{align}
        h(t;C_1)
