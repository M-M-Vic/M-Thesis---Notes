...
\begin{proof}
    ...
    $\theta_1=\theta_2=0$, both jockeying terms carry the factor $(x-y)$ and vanish, dragging the boundary term $\mu(x-y)P_y(y)$ with it; dividing the surviving terms by the common
    factor $z$ leaves
    \begin{equation*}
        \bigl[(\lambda_1+\lambda_2+\mu)z - \mu - (\lambda_1+\lambda_2)z^2\bigr]P(z,z)
        = -\mu(1-z)\,\pi(0,0).
    \end{equation*}
    Writing $\lambda=\lambda_1+\lambda_2$, the bracket factors,
    \begin{equation}
        (\lambda+\mu)x-\mu-\lambda z^{2}
        = -\bigl(\lambda z^{2}-(\lambda+\mu)z+\mu\bigr)
        = -(\lambda z-\mu)(z-1)
        = (\mu-\lambda z)(z-1);
        \nonumber
    \end{equation}
    cancelling $(z-1)$ against $-(1-z)=(z-1)$ on the right gives
    \begin{equation}
    ...
    \eqref{eq:B:Pxx} at $z=1$ with $P(1,1)=1-\pi_0$ gives
    ...
    The same identities therefore hold in the other no-abandonment model, Model-$B_2$;
    Model-$C_2$ retains the universal idle balance $\pi(0,0)=\rho\pi_0$~\eqref{eq:gen:pi00_general}
    but, with abandonment present, attains $\pi_0=1-\rho$ only in the limit
    $\theta_1\to0^+$.
\end{proof}
