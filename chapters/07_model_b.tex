...
\end{corollary}
The proof follows immediately by setting $x=y=z$ in the fundamental equation for Model-$B$ (Equation~\eqref{eq:gen:fundamental_equation}, with $\theta_1=\theta_2=0$): both jockeying terms, which carry the factor $(x-y)$, vanish, dragging the term $\mu(x-y)P_y(y)$ with them. The surviving diagonal equation is therefore \emph{identical} to that of Model-$A$ with $x=y=z$, so the proof of Corollary~\ref{cor:B:pi00} reduces to that of Corollary~\ref{cor:A:pi0}.
...
\begin{claim}\label{clm:B:volterra}
 Assume stability $\rho<1$. Along the characteristic lines $\gamma_2 x+\gamma_1 y=C_1$ of the fundamental equation~\eqref{eq:gen:fundamental_equation} (with abandonment parameters $\theta_1=\theta_2=0$ set to zero), parametrised by $y(x)=(C_1-\gamma_2 x)/\gamma_1$, the bivariate PGF of Model-$B$ admits the following integral representation:
    \begin{equation}
...
\begin{proof}[Proof of Claim~\ref{clm:B:volterra}]
    ...
    We now restore the right-hand side of the full equation~\eqref{eq:gen:fundamental_equation},
    ...
    Varying the constant in $P_h$ (variation of parameters, \S\ref{sssec:vop}) then
    gives the general solution of~\eqref{eq:B:ode_char} at once:
    \begin{equation}
