...
In this section we derive, via the Method of Characteristics, a general solution to the fundamental equation of this model. However this is provided in terms of the unknown boundary function $P_y(y)$, which requires solving a Volterra-type integral equation which, in case it has a closed-form solution, it is beyond the scope of this work. This section is left open but included for illustrative purposes, and motivates that we do not do the same analysis on Model-$X$, where the difficulty of having abandonments compounds over the difficulty of the problem of solving Model-$B$.

Model-$B$ introduces bidirectional jockeying ($\gamma_1,\gamma_2>0$) and no abandonment ($\theta_1=\theta_2=0$). Since jockeying preserves the total number of customers, the stationary distribution exists under the same condition as in Model-$A$, but it breaks the class-wise Poisson arrivals and the renewal structure, so the Pollaczek-Khinchine argument in \ref{sssec:A:probabilistic} is no longer available.
...
The following result is stated as a corollary, so that it stays at the same hierarchy level of the equivalent result for other models. However, since this model is not solved, there is no theorem to it.
...
For Model-$B$ we do not provide a solution; working the problem out, however,
exposes the precise analytical obstruction, which we record as a claim. To keep
the statement legible we do \emph{not} define here the auxiliary functions
$P_h$, $h$, $\mathcal{K}$ and $\mathcal{F}$ that enter it~---~each is constructed
explicitly in the proof~---~so that the \emph{structure} of the result is visible
at once.
\begin{claim}\label{clm:B:volterra}
    Assume stability, $\rho<1$. Along the characteristic lines
    $\gamma_2 x+\gamma_1 y=C_1$ of the fundamental
    equation~\eqref{eq:gen:fundamental_equation} (specialised to
    $\theta_1=\theta_2=0$), parametrised by $y(x)=(C_1-\gamma_2 x)/\gamma_1$, the
    joint PGF of Model-$B$ admits the integral representation
    \begin{equation}
        P\bigl(x,y(x)\bigr) \;=\; P_h\bigl(x,y(x)\bigr)\int_{0}^{x}
        \frac{h\bigl(t;C_1\bigr)}{P_h\bigl(t,y(t)\bigr)}\,dt,
        \label{eq:B:integral_rep}
    \end{equation}
    where $P_h$ is the (explicit) homogeneous solution and the forcing $h$ depends
    linearly on the unknown boundary trace $P_y(y)=P(0,y)$. Evaluating
    \eqref{eq:B:integral_rep} along the diagonal $x=y=z$, where
    $P(z,z)=\pi(0,0)/(1-\rho z)$ is known from Corollary~\ref{cor:B:pi00}, reduces
    the determination of $P_y$ to a \emph{Volterra integral equation of the first
    kind}
    \begin{equation}
        \mathcal{F}(z) \;=\; \int_0^z \mathcal{K}(z,t)\,P_y\!\bigl(y(t;z)\bigr)\,dt.
        \label{eq:B:volterra}
    \end{equation}
    Its kernel $\mathcal{K}$ does not factorise for general $(\gamma_1,\gamma_2)$,
    so~\eqref{eq:B:volterra} has no closed-form solution: the boundary trace $P_y$
    is at once the unknown of~\eqref{eq:B:volterra} and the input to the
    representation~\eqref{eq:B:integral_rep}. Model-$B$ is therefore left open.
\end{claim}
...
substituting $K$ and $M$, writing $\lambda=\lambda_1+\lambda_2$ and collecting the
two $C_1$-proportional terms over the common denominator $\gamma_1+\gamma_2$,
\begin{align}
    \begin{split}
        N &= -\gamma_1(\lambda+\mu)
        + C_1\,\frac{\lambda_2(\gamma_1+\gamma_2) + \gamma_1\lambda_1-\lambda_2\gamma_2}{\gamma_1+\gamma_2}
    ...
    \nonumber
\end{align}
using $\lambda_2(\gamma_1+\gamma_2)+\gamma_1\lambda_1-\lambda_2\gamma_2
...
\begin{equation}
    ...
    \nonumber
\end{equation}
the factor $\gamma_1^{\,N(C_1)/(\gamma_1(\gamma_1+\gamma_2))}$ depends on $(x,y)$ only through
$C_1$ and is absorbed, together with $f$, into a redefined arbitrary function
$F(C_1)$. Reading off the exponent of $(x-y)$ defines
\begin{equation}
    ...
    \label{eq:B:E}
\end{equation}
Exponentiating~\eqref{eq:B:lnP} therefore yields the general homogeneous solution
\begin{equation}
    ...
    \label{eq:B:Ph}
\end{equation}
where $F$ is an arbitrary differentiable function of the characteristic constant;
it cancels between the prefactor and the denominator of the integrand
in~\eqref{eq:B:integral_rep_body}, so the representation is independent of its
choice.

% ─────────────────────────────────────────────────────────────────────────────
\medskip\noindent\textbf{Step~2: The exponent $E(x,y)$ is strictly positive on $(0,1)^2$.}

Recall $\lambda = \lambda_1+\lambda_2$. Along any
fixed characteristic, $C_1 = \gamma_2 x + \gamma_1 y$ is constant, so the
exponent~\eqref{eq:B:E} depends only on $u := C_1\in(0,\gamma_1+\gamma_2)$:
\begin{equation*}
    E(u) = \frac{\lambda}{(\gamma_1+\gamma_2)^2}\,u - \frac{\lambda+\mu}{\gamma_1+\gamma_2} + \frac{\mu}{u}.
\end{equation*}
(For $(x,y)\in(0,1)^2$ we have $u = \gamma_2 x+\gamma_1 y \in (0,\gamma_1+\gamma_2)$; $E(u)$ is
continuous there, with $E(u)\to+\infty$ as $u\to0^+$ and $E(u)\to0^+$ as
$u\to(\gamma_1+\gamma_2)^-$, so it is finite at every interior point although not uniformly bounded.)
Multiplying through by
$u(\gamma_1+\gamma_2)^2>0$ gives the equivalent quadratic
\begin{equation*}
    q(u) := \lambda u^2 - (\lambda+\mu)(\gamma_1+\gamma_2) u + \mu(\gamma_1+\gamma_2)^2.
\end{equation*}
The sign of $E$ on $(0,\gamma_1+\gamma_2)$ equals the sign of $q(u)$ there.  We
factor $q$ by finding its roots.  The discriminant is
$\Delta = (\lambda+\mu)^2(\gamma_1+\gamma_2)^2 - 4\lambda\mu(\gamma_1+\gamma_2)^2 = (\gamma_1+\gamma_2)^2(\mu-\lambda)^2$,
so (using $\rho<1 \Rightarrow \mu>\lambda \Rightarrow \mu-\lambda>0$)
\begin{equation*}
    u_{1,2} = \frac{(\lambda+\mu)(\gamma_1+\gamma_2) \pm (\gamma_1+\gamma_2)(\mu-\lambda)}{2\lambda},
\end{equation*}
so that
\begin{equation*}
    u_1 = \frac{2\lambda(\gamma_1+\gamma_2)}{2\lambda} = \gamma_1+\gamma_2,
    \qquad
    u_2 = \frac{2\mu(\gamma_1+\gamma_2)}{2\lambda} = \frac{\gamma_1+\gamma_2}{\rho} > \gamma_1+\gamma_2.
\end{equation*}
Hence
\begin{equation*}
    q(u) = \lambda(u - (\gamma_1+\gamma_2))\!\left(u - \tfrac{\gamma_1+\gamma_2}{\rho}\right).
\end{equation*}
For every $u\in(0,\gamma_1+\gamma_2)$ both factors are strictly negative
($(u-(\gamma_1+\gamma_2))<0$ and $(u-(\gamma_1+\gamma_2)/\rho)<0$ since $(\gamma_1+\gamma_2)/\rho>(\gamma_1+\gamma_2)$),
so $q(u)=\lambda\cdot(\text{neg})\cdot(\text{neg})>0$, and therefore
\begin{equation*}
    E(x,y) > 0 \quad \text{for all } (x,y)\in(0,1)^2.
\end{equation*}
Note that $E\to0$ as $u\to(\gamma_1+\gamma_2)^-$ (i.e.\ as $(x,y)\to(1,1)$), while $E$ is
...
$y(t;z)-t=((\gamma_1+\gamma_2)/\gamma_1)(z-t)$, confirms that the $0\cdot\infty$ limit is
...
\end{equation*}
with $A$ as in~\eqref{eq:B:Adef}, while the forcing term collects the right-hand
side $G(x,y)$ of~\eqref{eq:gen:fundamental_equation},
\begin{equation}
    ...
    \label{eq:B:hdef}
\end{equation}
so that $h$ depends linearly on the unknown boundary trace $P_y(y)=P(0,y)$ and on
the known constant $\pi(0,0)=\rho(1-\rho)$ of Corollary~\ref{cor:B:pi00}.
