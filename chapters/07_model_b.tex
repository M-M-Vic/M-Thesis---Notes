...
\begin{proof}
    ...
    \end{equation}
    Writing $\lambda=\lambda_1+\lambda_2$, the bracket factors as $(\mu-\lambda x)(x-1)$;
...
\begin{align}
    ...
    \label{eq:B:split}
\end{align}
Each limit is a Hadamard finite part: the vanishing prefactor
$P_h^{\,\mathrm{diag}}(z):=\lim_{x\to z^-}P_h\bigl(x,y(x;z)\bigr)$ regularises the
divergent integral, and neither factor is meaningful in isolation.
...
Only the product limit~\eqref{eq:B:diag_limit} is a well-defined object; the vanishing
prefactor and the divergent integral acquire meaning solely through it. That this limit is
finite and equals the known diagonal value $\pi(0,0)/(1-\rho z)$ is not a fresh assumption
but was established in Step~2, where the asymptotic expansion
$y(t;z)-t\sim(\Gamma/\gamma_1)(z-t)$ as $t\to z^-$ matches the integrand's non-integrable
singularity $(y(t;z)-t)^{-1-E}$ against the vanishing of $P_h^{\,\mathrm{diag}}(z)$ at the
same order. The finite-part prescription therefore introduces no new analytic content: it is
a bookkeeping device for a limit already shown to exist, not a means of assigning a value to
a genuinely divergent expression. Writing the kernel
and known term \emph{formally} as
\begin{align}
...
% [AUTHOR: the regularisation of the 0*infinity limit (eq:B:split) -- giving the kernel K
% and known term F a rigorous finite-part meaning -- is itself the analytical obstruction.
% The displayed K, B are formal (the prefactor P_h^diag(z)=0 pointwise). A rigorous
% treatment (matched asymptotics near t=z, or a change of variable removing the
% (y-t)^{-1-E} singularity before passing to the limit) is what places Model-$B$ beyond
% scope. Flagged so this reads as a deliberate open end, not an oversight.]
The kernel $\mathcal{K}$ depends on $P_h$ through both arguments and does not simplify for
general $(\gamma_1,\gamma_2)$, so~\eqref{eq:B:rhs} does not yield a closed form.  Resolving
it would require Volterra inversion methods well beyond the scope of this thesis.
\end{proof}
