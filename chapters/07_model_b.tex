...
\begin{corollary}\label{cor:B:pi00}
    The empty-system probability $\pi_0$and the empty-queues probability $\pi(0,0)$ of Model-$B$ coincide with those of Model-$A$ baseline,
    \begin{equation}
...
\end{corollary}
The proof follows immediately by setting $x=y=z$ in the fundamental equation for Model-$B$ (Equation \eqref{eq:gen:fundamental_equation}, with $\theta_1=\theta_2=0$), as both jockeying terms with derivatives carry the factor $(x-y)$ vanish, also dragging the term $\mu(x-y)P_y(y)$ with it. The surviving diagonal equation is therefore \emph{identical} to that of Model-$A$ with $x=y=z$:, so the proof of Corollary~\ref{cor:B:pi00} reduces to the proof of Corollary~\ref{cor:A:pi0}.

For Model-$B$, we do not provide a solution. However, working it out illustrates the nature of the mathematical challenge it involves. The following claim is made:
\begin{claim}\label{clm:B:volterra}
    Assume stability, $\rho<1$, and write $\lambda:=\lambda_1+\lambda_2$ and
    $\Gamma:=\gamma_1+\gamma_2$. On each characteristic line
    $\gamma_2 x+\gamma_1 y=C_1$, with $C_1\in(0,\Gamma)$, of the fundamental
    equation~\eqref{eq:gen:fundamental_equation} (specialised to
    $\theta_1=\theta_2=0$), parametrised by
    \begin{equation}
        y(x)=\frac{C_1-\gamma_2 x}{\gamma_1},
        \label{eq:B:ychar}
    \end{equation}
    the bivariate PGF of Model-$B$ admits the integral representation
    \begin{equation}
        P\bigl(x,y(x)\bigr) \;=\; P_h\bigl(x,y(x)\bigr)\int_{0}^{x}
        \frac{h\bigl(t;\,C_1\bigr)}{P_h\bigl(t,\,y(t)\bigr)}\,dt,
        \label{eq:B:integral_rep}
    \end{equation}
    in which every constituent function is explicit, as follows.

    \smallskip\noindent\emph{(i)~Homogeneous factor.} The homogeneous solution is
    \begin{equation}
        P_h(x,y)=F(\gamma_2 x+\gamma_1 y)\,
        \exp\!\Bigl(\frac{\gamma_1\lambda_1-\lambda_2\gamma_2}{\gamma_1\Gamma}\,x\Bigr)\,
        x^{-\mu/(\gamma_2 x+\gamma_1 y)}\,
        (x-y)^{E(x,y)},
        \label{eq:B:Ph}
    \end{equation}
    where $F$ is an arbitrary differentiable function of the characteristic
    constant (it cancels between the prefactor and the denominator of the integrand
    in~\eqref{eq:B:integral_rep}, so the representation is independent of its
    choice), and the exponent is
    \begin{equation}
        E(x,y)=\frac{\lambda}{\Gamma^{2}}\,(\gamma_2 x+\gamma_1 y)
        -\frac{\lambda+\mu}{\Gamma}
        +\frac{\mu}{\gamma_2 x+\gamma_1 y}.
        \label{eq:B:E}
    \end{equation}

    \smallskip\noindent\emph{(ii)~Forcing term.} The forcing along the
    characteristic is
    \begin{equation}
        ...
        \label{eq:B:hdef}
    \end{equation}
    so that $h$ depends linearly on the unknown boundary trace $P_y(y)=P(0,y)$ and
    on the known constant $\pi(0,0)=\rho(1-\rho)$ of Corollary~\ref{cor:B:pi00}.

    \smallskip\noindent\emph{(iii)~Reduction to a Volterra equation.} Evaluating
    the representation~\eqref{eq:B:integral_rep} along the diagonal characteristic
    $C_1=\Gamma z$, which passes through $(z,z)$ and is parametrised by
    \begin{equation}
        y(t;z)=\frac{\Gamma z-\gamma_2 t}{\gamma_1},
        \label{eq:B:ydiag}
    \end{equation}
    and using the conservation identity $P(z,z)=\pi(0,0)/(1-\rho z)$ of
    Corollary~\ref{cor:B:pi00}, reduces the determination of $P_y$ to a
    \emph{Volterra integral equation of the first kind}
    \begin{equation}
        \mathcal{F}(z) \;=\; \int_0^z \mathcal{K}(z,t)\,P_y\!\bigl(y(t;z)\bigr)\,dt,
        \label{eq:B:volterra}
    \end{equation}
    whose kernel and data are, writing
    ...
    \begin{align}
        ...
        \label{eq:B:Fdef}
    \end{align}
    The prefactor $P_h^{\,\mathrm{diag}}(z)$ vanishes pointwise, so the kernel
    $\mathcal{K}$ and the known term $\mathcal{B}$ in~\eqref{eq:B:kernel}--\eqref{eq:B:Fdef}
    are to be read through the regularised (Hadamard finite-part) limit made
    precise in Step~5 of the proof. Every constituent
    of~\eqref{eq:B:kernel}--\eqref{eq:B:Fdef} is an explicit function of the
    parameters $(\gamma_1,\gamma_2,\lambda_1,\lambda_2,\mu)$ through the
    homogeneous solution~\eqref{eq:B:Ph}; the kernel $\mathcal{K}$ nonetheless does
    not factorise for general $(\gamma_1,\gamma_2)$, so~\eqref{eq:B:volterra} does
    not reduce to a closed-form solution for $P_y$. Model-$B$ is therefore left
    open.
\end{claim}
...
$F(C_1)$. Reading off the exponent of $(x-y)$ recovers the
function~\eqref{eq:B:E} of the claim,
\begin{equation*}
    ...
    + \frac{\mu}{\gamma_2 x+\gamma_1 y}.
\end{equation*}
Exponentiating~\eqref{eq:B:lnP} therefore yields the general homogeneous
solution~\eqref{eq:B:Ph} of the claim,
\begin{equation*}
    P_h(x,y) = F(\gamma_2 x+\gamma_1 y)
...
\end{equation*}
with $A$ as in~\eqref{eq:B:Adef}, while the forcing term $h(x;C_1)$ and the
inhomogeneity $G(x,y)$ are exactly those defined in~\eqref{eq:B:hdef} of the
claim.
...
along that curve is $y(t;z)=((\gamma_1+\gamma_2)z-\gamma_2 t)/\gamma_1$
(equation~\eqref{eq:B:ydiag}), so
...
The kernel and known term so extracted are \emph{formally} the functions
$\mathcal{K}$ of~\eqref{eq:B:kernel} and $\mathcal{B}$ of~\eqref{eq:B:Fdef}
...
\begin{align*}
    ...
    \mathcal{B}(z) &= \frac{\mu\cdot P_h^{\,\mathrm{diag}}(z)\cdot\pi(0,0)}{\gamma_1}
                        ...
                        \,dt,
\end{align*}
both \emph{to be read through the regularised limit~\eqref{eq:B:split} rather than
pointwise}; with $\mathcal{F}(z)=\pi(0,0)/(1-\rho z)-\mathcal{B}(z)$ as
in~\eqref{eq:B:Fdef}, equation~\eqref{eq:B:split} becomes the Volterra integral
equation~\eqref{eq:B:volterra} of the first kind,
