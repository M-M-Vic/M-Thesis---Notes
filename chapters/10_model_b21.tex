...
Model-$\BH$ simplifies the one-way jockeying model Model-$B_2$ in a single way. In
Model-$B_2$ \emph{every} waiting class-$1$ customer may jockey to the class-$2$ queue, so
the aggregate jockeying rate grows with the queue length, $\gamma_1 n_1$. Here only the
\emph{customer at the head of Queue~1} --- the one next in line for service --- may jockey,
at the fixed rate $\gamma_1$ whenever $n_1\ge1$; the aggregate rate is therefore the bounded
$\gamma_1\,\mathbf{1}_{\{n_1\ge1\}}$.

This one change is what makes the model tractable. The linear rate $\gamma_1 n_1$ turns the
joint-PGF equation of Model-$B_2$ into a first-order ODE in $x$, solved by the
integrating factor of \S\ref{sec:model_b2}. The flat rate contributes instead only the
\emph{algebraic} factor $\gamma_1 y(x-y)$ --- no derivative of $P$ survives --- so the
fundamental equation stays algebraic and yields to the Kernel Method exactly as in
Model-$A$ (\S\ref{sec:model_a}). The resulting PGF is structurally identical to
Model-$A$'s; only the kernel roots shift.
...
\end{align}
Jockeying is an out-transition from every state with $n_1\ge1$ (the $+\gamma_1$ on the left
of~\eqref{eq:B21:interior}--\eqref{eq:B21:xboundary}) and an in-transition into every state
with $n_2\ge1$ from $(n_1+1,n_2-1)$; it is absent when $n_2=0$ (no source state) or $n_1=0$
(no head to move), and all remaining transitions --- the $\lambda_1,\lambda_2$ arrivals and
the $\mu$ service completions --- are exactly those of Model-$A$.
...
$(x^*(y)-y)(\mu+\gamma_1 y)\,P_y(y) = \mu x^*(y)(1-y)\,\pi(0,0)$, whence the boundary
function is
\begin{equation}
    ...
    \label{eq:B21:Py}
\end{equation}
As a consistency check, at $y=0$ the denominator of~\eqref{eq:B21:Py} reduces to $\mu\,x^*(0)$
(with $x^*(0)>0$), so $P_y(0)=\pi(0,0)$, as it must.
...
\S\ref{sec:future_work}, which interpolates towards the length-proportional Model-$B_2$
(\S\ref{sec:model_b2}) as $k\to\infty$.
