...
\end{corollary}
The proof is the no-abandonment instance of Theorem~\ref{thm:gen:boundary} (Corollary~\ref{cor:gen:no_abandon}): head-of-line jockeying conserves the total count, so $\mathbb{E}[BP]=(\mu-\lambda)^{-1}$ and the baseline values hold.
...
\end{equation*}
and rearranging into a quadratic in $x$ gives exactly Equation~\eqref{eq:B21:quad},
\begin{equation*}
    \lambda_1 x^2 - \bigl[\lambda_1+\lambda_2(1-y)+\mu+\gamma_1\bigr]x + (\mu+\gamma_1 y) = 0,
\end{equation*}
whose two roots are
...
\end{equation}
Unlike Model-$A$'s Equation~\eqref{eq:A:xstar_prime}, the numerator carries an extra $\gamma_1$: the constant term $\mu+\gamma_1 y$ of the kernel~\eqref{eq:B21:quad} is itself $y$-dependent here.

Setting $x=x^*(y)$ in Equation~\eqref{eq:B21:fundamental} forces the right-hand side to vanish:$(x^*(y)-y)(\mu+\gamma_1 y)\,P_y(y) = \mu x^*(y)(1-y)\,\pi(0,0)$, whence the boundary function is
\begin{equation}
