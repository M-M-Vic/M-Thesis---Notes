...
Inspired by the work of Devos et al. \cite{devos2020alternating}, theis model —together with Model-$\CH$— eliminates the state-dependent jockeying rate for the class-$1$, only allowing the head-of-line customer from class-$1$ to transition at a fixed rate $\gamma_1$ whenever $n_1\geq1$, giving an aggregate rate $\gamma_1\,\mathbf{1}_{\{n_1\ge1\}}$.

This signifficantly simplifies the model. The flat rate replaces the derivative term in $x$ found in \S\ref{sec:model_b2} by an algebraic factor $\gamma_1 y(x-y)$ so the fundamental equation remains algebraic and can be solved by the Kernel Method exactly as in Model-$A$ (\S\ref{sec:model_a}).
...
The following theorem is the main result of this section; its proof is based the analytical (Kernel Method) treatment of Model-$A$ in~\S\ref{sssec:A:analytical}.
...
\begin{theorem}[Closed-form PGF for Model-$\BH$]\label{thm:B21:PGF}
    ...
    \begin{equation}
        ...
        \label{eq:B21:quad}
\end{equation}
with $x^*(y)$ the root inside the unit disk.
\end{theorem}

The following corollary is stated, for the sake of its result being at the same hierarchy level as homologous results for other models.
...
\end{corollary}
The proof is the same as for Corollary~\ref{cor:A:pi0}.
...
We apply the \emph{Kernel Method} to the fundamental equation~\eqref{eq:B21:fundamental}. Fix $y\in(0,1]$ and seek the values of $x$ at which the coefficient of $P(x,y)$ vanishes. Since $P(x,y)$ is a PGF it is bounded on the closed unit disk, so at any such root the right-hand side of~\eqref{eq:B21:fundamental} must also vanish. Dividing the kernel by $y\ne0$,
\begin{equation*}
