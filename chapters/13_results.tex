...
the two mechanisms. The exhaustive figure-by-figure evidence is collected in
...
the master chain cross-validates the CTMC itself to the $O(1/\sqrt{n})$ sampling floor. The
full visual evidence appears in Appendix~\ref{app:numerics:validation}.
...
($\rho_1/\rho\gtrsim0.6$, $\rho\lesssim0.6$); the validity map is
Figure~\ref{fig:val_ppgf} in Appendix~\ref{app:numerics:validation}.
...
so the approach is carried entirely by interior redistribution of mass. The full convergence
diagnostics are collected in Appendix~\ref{app:numerics:convergence}.
