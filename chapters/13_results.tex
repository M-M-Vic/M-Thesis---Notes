...
\hline
E-B  & Theorem~\ref{thm:B21:PGF} — Model-$B_2^1$  & B$_2^1$            & $P(x,y)$ on $5\times5$ grid                 & $<3\times10^{-7}$  & Accept \\
E-C  & Theorem~\ref{thm:C21:PGF} — Model-$C_2^1$ & C$_2^1$            & $P(x,y)$ on $5\times5$ grid                 & $<3\times10^{-11}$ & Accept \\
\hline
\end{tabular}
\end{table}
...
Model-$B_2^1$ gives $\mathbb{E}[N_1]=\rho\rho_B/(1-\rho_B)$ with
$\rho_B=\lambda_1/(\mu+\gamma_1)$ \eqref{eq:B21:EN1} and the conserved total
$\rho^2/(1-\rho)$, while Model-$C_2^1$ gives the rational $\mathbb{E}[N_1]$, $\mathbb{E}[N_2]$
...
test of the new transition structure underlying Models~$B_2^1$ and~$C_2^1$.
...
\begin{figure}[H]
  ...
  \caption{Simulation versus exact CTMC for the head-of-line models $B_2^1$
  ($\gamma_1=0.5$) and $C_2^1$ ($\theta_1=0.5$) at $\lambda_1=0.3$, $\lambda_2=0.4$,
...
\subsubsection{Head-of-line models \texorpdfstring{$B_2^1$}{B21} and \texorpdfstring{$C_2^1$}{C21}}

The two head-of-line experiment models must also recover Model-$A$ as their mechanism
parameter is switched off: as $\gamma_1\to0^+$ (Model-$B_2^1$) or $\theta_1\to0^+$
(Model-$C_2^1$) the kernel quadratic loses its $\gamma_1$/$\theta_1$ shift and the rational
...
\begin{figure}[H]
  ...
  ($\lambda_1=0.3$, $\lambda_2=0.4$, $\mu=1$). \emph{Left:} log--log plot of
  $\lVert\pi_{B_2^1}-\pi_A\rVert_1$ against $\gamma_1$, with the fitted first-order slope.
  \emph{Right:} $\lVert\pi_{C_2^1}-\pi_A\rVert_1$ against $\theta_1$, likewise first-order.
...
$B_2^1$ the conserved descriptors $\pi_0$ and $\pi(0,0)$ equal their Model-$A$ values
...
the two coordinates; for $C_2^1$ the descriptors $\pi_0$ and $\pi(0,0)$ relax onto the
...
slopes of $0.847$ ($C_2$), $0.903$ ($C_2^1$), $0.873$ ($B_2$) and $0.929$
($B_2^1$), each within the $0.1$--$0.2$ tolerance band of the theoretical exponent $1$. The
...
\begin{figure}[H]
  ...
  (2)~$\mathbb{E}[W_1]$ versus $\rho$ for all five solved models (A, $B_2$, $B_2^1$, $C_2$,
  $C_2^1$); (3)~$L_1$ convergence $C_2\to A$ and
