...
The method employed is a truncated CTMC solver: it solves the linear system $\pi Q=0$ directly, with no approximation in place beyond the finite truncation at $N_{\max}=40$. Each theoretical result is therefore verified against the stationary distribution at base parameters $\lambda_1=0.3$, $\lambda_2=0.4$, $\mu=1.0$, with the mechanism parameter ($\gamma_1$ or $\theta_1$) set to $0.5$ where applicable.

Table~\ref{tab:validation_summary} collects the verdict for every result. All mathematical results are accepted to within reasonable tolerance level: the lemma residuals and closed-form PGF errors fall between $10^{-7}$ and $10^{-15}$, and the corollary scatter lies on the diagonal to $10^{-4}$ across $60$ random configurations each. A discrete-event simulation of the master chain cross-validates the CTMC itself to the $O(1/\sqrt{n})$ sampling floor.
...
For each closed form validated, we compare the five solved models: Model-$A$ (baseline), Model-$B_2$ (class-$1$ jockeying), Model-$C_2$ (class-$1$ abandonment) and the two head-of-line variants Model-$\BH$ and Model-$\CH$. Each model produces a result wither in closed form or in integral form expressions for their bivariate PGF $P(x,y)$. Table~\ref{tab:comparison} collects the key structural facts across all five models.
...
Model-$A$ is a classic, well-studied model and its results are widespread. It operates as an ordinary two-class $M/M/1$ queue with non-preemptive priority. Its purpose here is to validate whether the other models converge into it when the parameters for the active mechanisms approach zero. All performance measures have a closed form; and, in the same fashion as the other models without abandonment, the total number of customers is also preserved.
