...
This proof follows Cohen~\cite{cohen1956delay}. It consists of keeping the class index $1$
 $n_1$ as a discrete parameter and forming a generating function only with respect
...
\begin{proof}
    We start by stating the balance equations for Model~$A$, with all jockeying and abandonment parameters set to zero (i.e. $\gamma_1=\gamma_2=\theta_1=\theta_2=0$) in the interioir balance equation (Equation \eqref{eq:gen:balance_interior}) and our $x$-boundary equation (Equation \eqref{eq:gen:balance_x_boundary}). We have
    \begin{align}
    ...
    \end{equation}
    The \emph{characteristic polynomial} (Equation \eqref{eq:A:ppgf_char}) is the reciprocal to that of the bivariate PGF approaches (i.e. the reciprocal of $\lambda_1 x^2-[\mu+\lambda_1+\lambda_2(1-y)]x+\mu$), thus its roots are the reciprocals of that kernel's roots: the positive is the reciprocal of the negative ($\widetilde{x}^{+}(y)=1/x^*(y)$), and conversely $\widetilde{x}^{-}(y)=1/x^{+}(y)$. By vieta's formulas, $\widetilde{x}^{-}(y)\,\widetilde{x}^{+}(y)=\lambda_1/\mu=\rho_1<1$. As seen in \S\ref{sssec:A:analytical}), only one cgaracteristic root —$\widetilde{x}^{-}(y)=1/x^{+}(y)$— lies inside the unit disk: for the bivariate PGF approach, that was the root with the negative sign $x^*(y)$; since $x^*(y)<1<x^{+}(y)$, we choose $\widetilde{x}^{-}(y)=1/x^{+}(y) \leq 1$.
    The selection of the negative sum is motivated by the fact that we need the sequence $\{\widetilde{P}(n_1,y)\}_{n_1\geq0}$ to be summable. In fact, we need
    \begin{equation*}
    ...
    \end{equation*}
    with the latter braking the condition $[\widetilde{x}^{+}(y)]^{n_1}$, yielding a non-summable sequence. We will therefore difine $\widetilde{x}^*(y):=\widetilde{x}^{-}(y)$.
    \begin{equation*}
        ...
        = \rho_1\,x^*(y).
    \end{equation*}
    The last identity holds due to the fact that $\widetilde{x}^*$ and $x^*$ differ only in the denominator. Next,
    \begin{equation}
    ...
    Sunning the geometric recurrence (Equation \eqref{eq:A:ppgf_geometric}) over $n_1 \geq 0$ in for $x \in (0,1]$ recovers
    \begin{equation}
    ...
    \end{equation}
    with $x^*(y)$ and $x^{+}(y)$ are the roots found in \S\ref{sssec:A:analytical}. By Vieta's product formulas, we have $x^*(y)\,x^{+}(y)=\mu/\lambda_1=1/\rho_1$. This, together with $\widetilde{x}^*(y)$ (Equation \eqref{eq:A:xtildestar}), gives $\widetilde{x}^*(y)=\rho_1 x^*(y)=1/x^{+}(y)$.
