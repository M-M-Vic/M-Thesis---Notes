...
solution in the priority index.

\paragraph{Reduced balance equations.}
Setting $\gamma_1=\gamma_2=\theta_1=\theta_2=0$ in the interior
equation~\eqref{eq:gen:balance_interior} and in the $x$-boundary
equation~\eqref{eq:gen:balance_x_boundary} gives, for Model~$A$,
\begin{align}
...
\paragraph{Recurrence in $n_1$.}
Fix $n_1\geq1$. Multiplying the interior equation~\eqref{eq:A:ppgf_interior} by $y^{n_2}$
and summing over $n_2\geq1$ yields
\begin{equation}
    ...
    \label{eq:A:ppgf_summed}
\end{equation}
Reindexing the last sum ($m=n_2-1$) gives
$\lambda_2\sum_{n_2\geq1}\pi(n_1,n_2-1)y^{n_2}=\lambda_2 y\,\widetilde{P}(n_1,y)$.
Completing the remaining partial sums to full sums by adding and subtracting the
$n_2=0$ terms, $\sum_{n_2\geq1}\pi(n_1,n_2)y^{n_2}=\widetilde{P}(n_1,y)-\pi(n_1,0)$, turns
\eqref{eq:A:ppgf_summed} into
\begin{equation}
    ...
    \label{eq:A:ppgf_withboundary}
\end{equation}
Collecting the bare boundary terms of~\eqref{eq:A:ppgf_withboundary} on the right-hand
side leaves
\begin{equation*}
    ...
    = (\lambda_1+\lambda_2+\mu)\pi(n_1,0)
      -\mu\pi(n_1+1,0)-\lambda_1\pi(n_1-1,0).
\end{equation*}
The right-hand side vanishes identically: by the $x$-boundary
equation~\eqref{eq:A:ppgf_xbound}, $\mu\pi(n_1+1,0)+\lambda_1\pi(n_1-1,0)
=(\lambda_1+\lambda_2+\mu)\pi(n_1,0)$. The boundary terms therefore cancel exactly,
and we obtain the homogeneous recurrence
\begin{equation}
...
Equation~\eqref{eq:A:ppgf_recurrence} is a second-order linear recurrence in $n_1$ with
coefficients constant in $n_1$, so its general solution is a linear combination of the
two geometric sequences $[\widetilde{x}^{\pm}(y)]^{n_1}$, where $\widetilde{x}^{\pm}(y)$
...
\end{equation}
The characteristic polynomial~\eqref{eq:A:ppgf_char} is the reciprocal of the kernel
polynomial $\lambda_1 x^2-[\mu+\lambda_1+\lambda_2(1-y)]x+\mu$ of the analytical
approach; hence its roots are the reciprocals of the kernel roots,
$\widetilde{x}^{+}(y)=1/x^*(y)$ and $\widetilde{x}^{-}(y)=1/x^{+}(y)$. By
Vieta's formulas, $\widetilde{x}^{-}(y)\,\widetilde{x}^{+}(y)=\lambda_1/\mu=\rho_1<1$.
Since $x^*(y)<1<x^{+}(y)$ for $y\in[0,1)$ (shown in
\S\ref{sssec:A:analytical}), exactly one characteristic root lies in the open unit disk,
namely the negative-sign root $\widetilde{x}^{-}(y)=1/x^{+}(y)$.

Selection of this root is forced by summability rather than by analyticity on $|x|<1$.
For $y\in[0,1]$ the terms $\widetilde{P}(n_1,y)=\sum_{n_2\geq0}\pi(n_1,n_2)y^{n_2}$ are
nonnegative, and
\begin{equation*}
    \sum_{n_1=0}^{\infty}\widetilde{P}(n_1,y) = P(1,y) \le P(1,1) = 1-\pi_0 = \rho < \infty,
\end{equation*}
so the sequence $\{\widetilde{P}(n_1,y)\}_{n_1\geq0}$ must be summable. The mode
$[\widetilde{x}^{+}(y)]^{n_1}$ with $\widetilde{x}^{+}(y)\geq1$ is not summable (at $y=1$
the two roots are $\widetilde{x}^{-}(1)=\rho_1$ and $\widetilde{x}^{+}(1)=1$, the latter
giving a non-summable constant sequence), so its coefficient must vanish. Writing
$\widetilde{x}^*(y):=\widetilde{x}^{-}(y)$,
\begin{equation}
    \widetilde{x}^*(y) = \frac{\bigl(\mu + \lambda_1 + \lambda_2(1-y)\bigr)
            - \sqrt{\bigl(\mu + \lambda_1 + \lambda_2(1-y)\bigr)^2 - 4\lambda_1\mu}}{2\mu}
    = \rho_1\,x^*(y),
    \label{eq:A:xtildestar}
\end{equation}
the last identity because $\widetilde{x}^*$ and $x^*$ share the same numerator and differ
only in the denominator ($2\mu$ versus $2\lambda_1$). Consequently
\begin{equation}
...
At $y=1$ the surviving root recovers the class-$1$ traffic intensity,
\begin{equation}
    \widetilde{x}^*(1) = \frac{\lambda_1}{\mu} = \rho_1,
    \label{eq:A:xtildestar_at1}
\end{equation}
so that, by~\eqref{eq:A:ppgf_geometric},
$\widetilde{P}(n_1,1)=\widetilde{P}(0,1)\,\rho_1^{n_1}$. Since
$\widetilde{P}(0,1)=P_y(1)=\rho(1-\rho_1)$ (cf.~\eqref{eq:A:Py1}), this is the geometric
marginal
$\mathbb{P}(N_1=n_1,\ \text{busy})=\rho(1-\rho_1)\rho_1^{n_1}$
of the priority class, as expected.

\paragraph{Recovery of $P(x,y)$.}
The constant $\widetilde{P}(0,y)$ requires no further work: by definition
\begin{equation*}
    \widetilde{P}(0,y) = \sum_{n_2=0}^{\infty}\pi(0,n_2)\,y^{n_2} = P(0,y) = P_y(y),
\end{equation*}
which was determined in the two preceding approaches,
$P_y(y)=\dfrac{x^*(y)(1-y)}{x^*(y)-y}\,\rho(1-\rho)$ (Equation~\eqref{eq:A:Py}). Summing
the geometric law~\eqref{eq:A:ppgf_geometric} over $n_1$ then reconstructs the joint PGF,
for $|x|\le1$,
\begin{equation}
...
\end{equation}
It remains to bring~\eqref{eq:A:ppgf_reconstruct} to the exact form of
Theorem~\ref{thm:A:PGF}. Writing the kernel of the fundamental
equation~\eqref{eq:A:fundamental}, divided by $\mu$, as
\begin{equation}
    K(x,y) := (1+\rho_1+\rho_2)xy - y - \rho_1 x^2 y - \rho_2 x y^2
            = -y\rho_1\bigl(x-x^*(y)\bigr)\bigl(x-x^{+}(y)\bigr),
    \label{eq:A:kernel_factored}
\end{equation}
where $x^*(y)$ and $x^{+}(y)$ are the two kernel roots of \S\ref{sssec:A:analytical},
with Vieta product $x^*(y)\,x^{+}(y)=\mu/\lambda_1=1/\rho_1$. By~\eqref{eq:A:xtildestar}
together with $x^*x^{+}=1/\rho_1$, the admissible characteristic root satisfies
$\widetilde{x}^*(y)=\rho_1 x^*(y)=1/x^{+}(y)$, whence
\begin{equation*}
    1-\widetilde{x}^*(y)\,x = 1 - \frac{x}{x^{+}(y)} = \frac{x^{+}(y)-x}{x^{+}(y)}
    = \frac{K(x,y)}{y\rho_1\,x^{+}(y)\bigl(x-x^*(y)\bigr)},
\end{equation*}
the last equality by the factorization~\eqref{eq:A:kernel_factored}. Substituting this and
$P_y(y)=\frac{x^*(y)(1-y)}{x^*(y)-y}\rho(1-\rho)$ into~\eqref{eq:A:ppgf_reconstruct}, and
using $\rho_1\,x^*(y)\,x^{+}(y)=1$,
\begin{align*}
    P(x,y)
...
\end{align*}
which is precisely Equation~\eqref{eq:A:PGF}. This third route therefore reproduces
Theorem~\ref{thm:A:PGF}; the same recurrence structure in the discrete index reappears,
in inhomogeneous form, in state space $\widetilde{S}$ below.
