...
The balance equations are given —Equations \eqref{eq:gen_balance_interior}, \eqref{eq:gen_balance_x-boundary}, \eqref{eq:gen_balance_y-boundary}, \eqref{eq:gen_balance_zero} and \eqref{eq:gen_balance_idle} for state space $S$, and Equations \eqref{eq:tilde_balance_interior}, \eqref{eq:tilde_balance_diagonal}, \eqref{eq:tilde_balance_y-boundary}, \eqref{eq:tilde_balance_zero} and \eqref{eq:tilde_balance_idle} for state space $\widetilde{S}$ by plugging $\gamma_1=0$ and $\gamma_2=0$. The derivation follows accordingly, hence the starting point for the analysis of the model is the equation for the dynamics of our (P)PGF: this is Equation \eqref{eq:gen_fundamental_equation} for state space $S$ and Equation \eqref{eq:gen_PPGF_dynamics} for state space $\widetilde{S}$, for $\gamma_1=\gamma_2=0$. The corresponding subsections will introduce the "$\gamma$-less" versions of these equations.
...
We start by stating a $\gamma$-free version of Equation \eqref{eq:gen_fundamental_equation}, where the function $P_y(y)$ and the constant $\pi(0,0)$ are still to be determined:
\begin{align}
    \begin{split}
...
\begin{proof}
    ...
    \begin{equation}
        \frac{d}{dy}x^*(1) = \frac{\rho_2}{1-\rho_1}. \label{eq:xstar_1}
    \end{equation}
    ...
    \end{align*}
    Thus, we find $\pi(0,0) = \rho(1-\rho)$, and since $\pi_0 = 1-\rho$ holds for all no-abandonment models (Equation~\eqref{eq:pi0_noaband}), this establishes Corollary~\ref{cor:A:pi0}. For reference:
    \begin{equation}
        \pi(0,0) = \rho (1-\rho). \label{eq:pi00}
    \end{equation}

    Now that we have determined $P_y(y)$ and $\pi(0,0)$, substituting Equation \eqref{eq:pyy_xstars} and Equation \eqref{eq:pi00} into Equation \eqref{eq:n1n2_model-A_dynamics} yields
    \begin{align*}
        \left[ 
...
\begin{proof}
    Our goal here is to derive the partial PGF $P_y(y)$ based on probabilistic arguments. As done in the analytical approach, we start from the fundamental global balance equation for Model~A, i.e.\ Equation \eqref{eq:n1n2_model-A_dynamics}:
    \begin{equation}
        \left[ (\lambda_1 + \lambda_2 + \mu) xy - \mu y - \lambda_1 x^2 y - \lambda_2 x y^2 \right] P(x,y) =
...
Note that we can use the $x$-boundary equation (Equation \eqref{eq:gen_balance_x-boundary}) to simplify the expression. Doing so and then grouping terms yields
\begin{equation}
...
A similar procedure can be replicated with the $y$-boundary balance equation (Equation \eqref{eq:gen_balance_y-boundary}) to obtain $\widetilde{P}(0,y)$. First, solving the characteristic equation yields two roots:
\begin{equation*}
...
\begin{theorem}[Approximate PPGF on $\widetilde{S}$]\label{thm:A:PPGF_Stilde}
    Under the approximation that neglects the boundary-diagonal forcing term $\mu y^n(1-y)\,\tpi(n+1,n+1)$ in~\eqref{eq:n1n_model-A_dynamics}, the homogeneous recurrence admits the geometric solution
    \begin{equation}
