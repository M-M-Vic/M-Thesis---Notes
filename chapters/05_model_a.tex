...
The mean numbers of waiting customers follow from $P(x,y)$ by differentiation.
The class-1 marginal $P(x,1)=\rho(1-\rho_1)/(1-\rho_1 x)$ is a geometric PGF,
yielding the closed-form expressions
\begin{align}
    \mathbb{E}[N_1] &= \frac{\rho\rho_1}{1-\rho_1}, \label{eq:A:EN1} \\
    \mathbb{E}[N_2] &= \frac{\rho\rho_2}{(1-\rho_1)(1-\rho)}. \label{eq:A:EN2}
\end{align}
Their sum is $\mathbb{E}[N_1]+\mathbb{E}[N_2] = \rho^2/(1-\rho)$, the
standard $M/M/1$ queue-length result for the combined system.

% [Expand: derive \eqref{eq:A:EN2} from the P-K formula.
%  The class-2 subsystem is M/G/1 with load ρ_B = λ₂ E[B] = ρ₂/(1-ρ₁), giving
%  E[N₂] = ρ_B/(1-ρ_B) = ρ₂(1-ρ₁)⁻¹ / (1-ρ₂(1-ρ₁)⁻¹) = ρ₂/((1-ρ₁)-ρ₂) = ρ₂/(1-ρ₁-ρ₂)·... check.]

\textbf{Little's Law} ($\mathbb{E}[N_i] = \lambda_i\,\mathbb{E}[W_i]$) holds for
any stable system regardless of the arrival distribution;
since arrivals are Poisson here, it also coincides with the PASTA-based
decomposition~\eqref{eq:prio:py_decomp}.  The mean waiting times in queue are
\begin{align}
    \mathbb{E}[W_1] &= \frac{\rho}{\mu(1-\rho_1)}, \label{eq:A:EW1} \\
    \mathbb{E}[W_2] &= \frac{\rho}{\mu(1-\rho_1)(1-\rho)}. \label{eq:A:EW2}
\end{align}
Class-1 benefits from priority: its waiting time is independent of $\rho_2$
(only the total load $\rho$ and its own load $\rho_1$ appear).
Class-2 pays a factor $1/(1-\rho_1)$ on top of the single-class waiting time.
