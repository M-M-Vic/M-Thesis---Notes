...
We now introduce a theorem, and the proofs for it —one analytical and one probabilistic— will follow in the subsections below.
...
\begin{proof}
    We apply the \textit{kernel method}: fix $y\in[0,1)$ and find $x=x^*(y)$ inside the unit disc such that the bracket on the left of Equation~\eqref{eq:A:fundamental} vanishes. At any such root the left-hand side is zero, so the right-hand side must vanish as well:
    \[
        (x^*(y)-y)\,P_y(y) \;=\; x^*(y)(1-y)\,\pi(0,0).
    \]
    Dividing both sides by $y\neq 0$ in Equation~\eqref{eq:A:fundamental} and setting the coefficient of $P$ to zero gives
    \begin{align}
        \begin{split}
...
\begin{proof}
    ...
    In the $M/G/1$ equivalent, having $k\geq1$ customers in the system corresponds to one class-$2$ customer in service and $k-1$ class-$2$ customers waiting, which in the original model is exactly the event $\{N_1=0,\, N_2=k-1,\, \text{busy}\}$. Since $L_2(y)$ is the PGF of the $M/G/1$ system count and the idle probability of the $M/G/1$ (the $y^0$ coefficient) equals $(1-\rho)/(1-\rho_1)=\pi(0,0)/[\rho(1-\rho_1)]$, the above identification yields:
    \begin{equation}
        \mathbb{E}[y^{N_2} \mid \text{busy}, N_1=0] = L_2(y) = \frac{1-\rho}{1-\rho_1} \cdot \frac{x^*(y)(1-y)}{x^*(y) - y}.
        \label{eq:A:L2_conditional}
    \end{equation}

    For the prefactor, the non-preemptive priority structure ensures that class-$1$ evolves as an independent $M/M/1(\lambda_1,\mu)$ queue, so the marginal distribution of $N_1$ conditioned on the server being busy is geometric with parameter $\rho_1$; in particular $\mathbb{P}(N_1=0\mid\text{busy})=1-\rho_1$. Therefore $\mathbb{P}(\text{busy},N_1=0)=\rho(1-\rho_1)$.
...
\end{equation}
consistent with the geometric marginal $P(N_1=n_1,\text{busy})=(1-\rho_1)\rho_1^{n_1}\rho$ of the priority class. Determining $\widetilde{P}(0,y)$ from the $y$-boundary equation~\eqref{eq:gen:balance_y_boundary} and reconstructing $P(x,y)$ via $P(x,y)=\sum_{n_1=0}^\infty \widetilde{P}(n_1,y)\,x^{n_1}$ then reproduces Theorem~\ref{thm:A:PGF}, as verified by the analytical proof above.
...
\end{equation}
The inhomogeneous term $\mu y^n(1-y)\tpi(n+1,n+1)$ prevents the direct application of Cohen's geometric-sequence ansatz $\tP(y,n)=\tP(y,0)\cdot [f(y)]^n$. However, since $\tpi(n+1,n+1)$ decays geometrically in $n$ under stability, this forcing term is exponentially small, and we obtain a useful approximation by neglecting it. The resulting homogeneous recurrence is
\begin{equation}
