...
Model~$A$ is the baseline of the hierarchy: all extra parameters are zero, leaving a pure two-class non-preemptive priority queue with no jockeying and no abandonment.  The analysis is carried out on both state spaces $S$ and $\widetilde{S}$; on $S$ two complete proofs (analytical and probabilistic) and a PPGF-based outline are presented.
...
\textbf{Tractability.} With no jockeying, class-1 customers have strict non-preemptive priority: upon any service completion, the server always picks a waiting class-1 customer next. Consequently, class-2 customers are blocked whenever $N_1>0$; their effective service time is the duration of a complete class-1 busy period $B$ of the $M/M/1(\lambda_1,\mu)$ sub-queue, making the class-2 subsystem equivalent to an $M/G/1(\lambda_2,B)$ queue. This reduction enables both the analytical kernel-method approach (zero of the coefficient polynomial) and the probabilistic Pollaczek--Khinchine route. A PPGF-based approach on $S$ is also outlined for comparison.
...
\begin{proof}[Proof of Corollary~\ref{cor:A:pi0}]
    \textbf{Diagonal PGF.}  Setting $x=y=z$ in the theorem formula: substituting $x=y=z$ into the kernel equation~\eqref{eq:A:xstar} with $y=z$ gives $\lambda z^2-(\mu+\lambda)z+\mu=(z-1)(\lambda z-\mu)=0$, so $x^*(z)=z$ only at $z\in\{1,1/\rho\}$, neither of which lies in $(0,1)$ for $\rho<1$.  Therefore $(z-x^*(z))/(x^*(z)-z)=-1$ throughout $(0,1)$, while the denominator becomes
    \[
...
\begin{proof}
    ...
    \end{align*}
    To confirm that $x^*(y)<1$ for all $y\in[0,1)$: the derivative formula above gives
    \[
    ...
    \]
    since $(\mu+\lambda_1+\lambda_2(1-y))^2>(\mu+\lambda_1+\lambda_2(1-y))^2-4\lambda_1\mu$ whenever $\lambda_1\mu>0$.  Because $x^*$ is strictly increasing on $[0,1]$ with $x^*(1)=1$, we have $x^*(y)<1$ for all $y\in[0,1)$.  By Vieta's formulas for the kernel quadratic $\lambda_1 x^2-[\mu+\lambda_1+\lambda_2(1-y)]x+\mu=0$, the product of roots equals $\mu/\lambda_1>1$; hence $x^+(y)=\mu/(\lambda_1 x^*(y))>1$ throughout $[0,1)$.  Therefore $x^*(y)$ is the unique root strictly inside the unit disc, and $x^+(y)>1$ is inadmissible. Hence
    \begin{equation}
        x^*(y) = \frac{
