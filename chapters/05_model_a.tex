...
\end{equation}
This coincides with Theorem~\ref{thm:A:PGF}. Indeed, using
$\widetilde{x}^*(y)=1/x^{+}(y)$ and the kernel Vieta relation
$x^*(y)\,x^{+}(y)=\mu/\lambda_1=1/\rho_1$,
\begin{equation*}
    P(x,y)
    = \frac{x^{+}(y)\,P_y(y)}{x^{+}(y)-x}
    = \frac{x^*(y)\,x^{+}(y)\,(1-y)\,\rho(1-\rho)}{\bigl(x^*(y)-y\bigr)\bigl(x^{+}(y)-x\bigr)}
    = \frac{(1-y)\,\rho(1-\rho)}{\rho_1\bigl(x^*(y)-y\bigr)\bigl(x^{+}(y)-x\bigr)},
\end{equation*}
which equals~\eqref{eq:A:PGF} after factoring its kernel as
$(1+\rho_1+\rho_2)xy-y-\rho_1 x^2y-\rho_2 xy^2=-y\rho_1\bigl(x-x^*(y)\bigr)\bigl(x-x^{+}(y)\bigr)$.
This recurrence structure in the discrete index reappears, in inhomogeneous form, in
