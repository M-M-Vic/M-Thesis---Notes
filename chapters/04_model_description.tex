...
Figure~\ref{fig:models-queue-overview} provides a visual overview of Model-$X$ and each of the variants that we will consider, but omits the two head-of-line model, as it is less conventional and a diagram like these would not be of much help:
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{%
...
The model discussed in this section is an $M/M/1$ queueing system with two priority classes, where customers are served on a First-Come, First-Served (FCFS) basis within their own class and those coming from the priority class (class-$1$) have non-preemptive priority over those of the non-priority class (class-$2$); that is, when a class-$2$ customer is in service, that service will not be interrupted by apotential arrival of a class-$1$ customer. Beyond basic dynamics, Model-$X$ incorporates two distinct queue-leaving mechanisms. First, \emph{jockeying}: a waiting customer can abandon its own queue and \emph{join the other queue}, at a per-customer rate $\gamma_1$ for direction $1\to2$ and $\gamma_2$ for direction $2\to1$, so the total number of customers is preserved. Second, \emph{abandonment} (reneging): a waiting customer may completely leave the \emph{system} at a rate per-customer $\theta_i$, so the total number of customers decreases by one. Concretely, each Class-$i$ customer that is waiting in line (i.e.\ not the one in service) holds an independent $\mathrm{Exp}(\theta_i)$ patience clock and abandons the system when it expires; consequently, the aggregate class-$i$ abandonment rate in a state with $n_i$ waiting customers is the linear term $\theta_i n_i$ that appears in the balance equations. This is precisely the linear-reneging mechanism of the $M/M/1+M$ queue introduced in \S\ref{ssec:erlangA}.
...
\end{itemize}
Note that the variables $N_1$, $N_2$ and $N$ count customers in the \emph{queues}, not the \emph{system}. Since service rate is common to both classes, we denote the idle system state as $(0)$ and all other states by $(n_1,n_2)$, so state $(0,0)$ implies someone is in service and the queues are empty. Alternatively, we can define another state space by total queue depth using an auxiliary state space $\widetilde{S}$, tracking only $n_2$ and $n = n_1 + n_2$ with the constraint $n \geq n_2 \geq 0$. We define:
\begin{align}
    \begin{split}
...
For these state spaces, we define the long-run probabilities of finding the system in that state as follows:
\begin{itemize}
...
We assume Poisson arrivals for each class and exponential service times (common across classes). We adopt the following notation:
\begin{itemize}
