...
\end{equation}
The usefulness of $\widetilde{S}$ will turn out to be confined to Model-$A$; the rationale for introducing it, and the reason it fails to simplify any model with an active mechanism, are documented in Remark~\ref{rem:gen:Stilde_failed} at the end of this section.
...
The subscript in the variant names records which rate of a mirrored pair is switched off: Model-$B_2$ sets $\gamma_2=0$, retaining only the $1\to2$ jockeying direction, and Model-$C_2$ sets $\theta_2=0$, retaining only class-$1$ abandonment. The mirrored variants Model-$B_1$ ($\gamma_1=0$) and Model-$C_1$ ($\theta_1=0$), not treated in this thesis, are named symmetrically.
