\section{The general model (Model-\texorpdfstring{$X$}{X}) and its fundamental equation}\label{sec:model_x}
...
The fundamental equation already delivers the first specialisation of Theorem~\ref{thm:gen:boundary}: the case of no abandonment, which covers Models~$A$, $B$, $B_2$ and $\BH$ at once.

\begin{corollary}[Empty states without abandonment]\label{cor:gen:no_abandon}
    Suppose $\theta_1=\theta_2=0$ (Models~$A$, $B$, $B_2$ and $\BH$) and $\rho<1$. Then the total number in the system is a birth--death process with birth rate $\lambda$ and death rate $\mu$ --- an $M/M/1$ queue --- for \emph{any} jockeying rates $\gamma_1,\gamma_2$, since jockeying merely relocates a waiting customer and so conserves the total count. Its mean busy period is $\mathbb{E}[B]=(\mu-\lambda)^{-1}$, and Theorem~\ref{thm:gen:boundary} gives
    \begin{equation}
        \pi_0 = 1-\rho, \qquad \pi(0,0)=\rho(1-\rho).
        \label{eq:gen:pi0_noabandon}
    \end{equation}
    Moreover, setting $x=y=z$ in the fundamental equation~\eqref{eq:gen:fundamental_equation} annihilates both convection terms (each carries the factor $x-y$), leaving the algebraic relation $-(1-z)\bigl(\mu-\lambda z\bigr)P(z,z)=-\mu(1-z)\,\pi(0,0)$, whence the diagonal of the PGF and the normalisation read
    \begin{equation}
