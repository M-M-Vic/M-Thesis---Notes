...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{figures/models-overview.tikz}%
    }
    \caption{State-transition diagrams on state space $S$ for Model~X (parent, top)
    and its four specializations (bottom row), showing representative states
    $(0,0)$, $(n_1,0)$, $(n_1,n_2)$, and $(0,n_2)$.
    \textbf{Solid} arrows: Poisson arrivals ($\lambda_i$) and exponential service ($\mu$),
    present in every model.
    \textbf{Dashed} arrows: jockeying ($\gamma_i n_i$), which transfers a waiting
    customer between queues and conserves the total~$n$.
    \textbf{Dotted} arrows: abandonment ($\theta_i n_i$), which removes a waiting
    customer from the system and reduces~$n$ by one.}
