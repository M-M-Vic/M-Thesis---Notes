...
\begin{figure}[H]
    ...
    \caption{Physical queue layout for Model~X (top) and its four specializations
    (bottom row). Queue~1 (top buffer) has non-preemptive priority over Queue~2.
    \textbf{Solid}: Poisson arrivals ($\lambda_i$) and exponential service ($\mu$).
    \textbf{Dashed}: jockeying ($\gamma_i$), a transfer between queues that conserves
    the total number of customers.
    \textbf{Dotted}: abandonment ($\theta_i$), a true departure that reduces the total.}
