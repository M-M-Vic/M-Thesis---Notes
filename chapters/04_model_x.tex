...
\begin{itemize}
    ...
    \item $\gamma_2$: per-customer rate at which a waiting Class-$2$ customer jockeys
          ...
          \emph{conserves} the total $n=n_1+n_2$ (Model~$B_2$ sets $\gamma_2=0$);
...
\textbf{Stability.}
In the absence of abandonment ($\theta_1=\theta_2=0$, i.e.\ Models $A$, $B$ and $B_2$), the
...
only one class reneges —as in Model~$C_2$, where $\theta_1>0$ but $\theta_2=0$—
...
waiting customers), so~\eqref{eq:gen:pi00_general} is unaffected by $\gamma_i$ and
$\theta_i$. What \emph{does} depend on the abandonment parameters is the value of
$\pi_0$ itself, as we now discuss.

\emph{Models without abandonment ($A$, $B$, $B_2$).} When $\theta_1=\theta_2=0$, jockeying
...
\emph{Models with abandonment ($X$, $C_2$).} As soon as a true abandonment is present, the
...
whenever $\theta_1\neq\theta_2$ —in particular for Model~$C_2$, where $\theta_2=0$.
...
single-class-abandonment case $\theta_2=0$, in the dedicated analysis of Model~$C_2$.

In all cases, the mass on the busy states is
\begin{equation}
    \sum_{n_1=0}^{\infty}\sum_{n_2=0}^{\infty}\pi(n_1,n_2) = 1-\pi_0,
    \label{eq:gen:busy_mass}
\end{equation}
which equals $\rho$ only in the non-abandonment case~\eqref{eq:gen:pi0_noaband}; for
Models $X$ and $C_2$ one has $1-\pi_0\neq\rho$ in general. This conditioning on a busy
server will be relevant in our derivations, as we will look at the queue length
conditioned on the server being busy.

\subsection{Balance equations}

This subsection presents the state-transition diagrams of Model~$X$ on both state spaces $S$ and $\widetilde{S}$, and derives the corresponding balance equations.
