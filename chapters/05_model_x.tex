...
($\theta_1,\theta_2>0$), and from which every other model is recovered by
...
abandonment, though it keeps each arrival stream Poisson, couples the two queues through
the state-dependent rates $\theta_i n_i$.

The analysis therefore proceeds analytically. The method of characteristics yields the
homogeneous solution of the fundamental equation; variation of parameters gives the
general solution; the boundary behaviour at $x=0$ pins the free function; and evaluating
along the characteristic through a diagonal point reduces the determination of the unknown
boundary function $P_y(y)$ to a Volterra integral equation of the first kind.

There the analysis stops, for two reasons established below: the kernel of that equation
is built from a non-elementary quadrature along transcendental characteristics, and its
right-hand side is itself coupled to an unknown diagonal gradient. Model-$X$ is therefore
left open; its role in the hierarchy is to expose the precise obstructions that its
tractable specialisations circumvent, and its queue layout is the left panel of
...
in every direction of the $(n_1,n_2)$-plane while the total arrival rate stays fixed at
$\lambda=\lambda_1+\lambda_2$: far from the origin the chain drifts strictly towards
smaller queues, so for any $\theta_1,\theta_2>0$ no traffic-intensity restriction of the
form $\rho<1$ is to be expected (cf.\ the Erlang-A queue, \S\ref{ssec:erlangA}, which is
stable at all loads). Throughout this section we therefore work under the hypothesis
that the chain is positive recurrent, so that the stationary distribution exists.
Setting $\theta_1=\theta_2=0$ removes this unbounded downward
...
Model-$X$ is therefore left open. Its role in the hierarchy is to make the
...
solvable baseline Model-$A$ (\S\ref{sec:model_a}). The full model~$X$, with
all mechanisms active, retains both obstructions simultaneously and is the one case that does
not close.
